...
if TYPE_CHECKING:
    ...
    from observations.eyes.base import ObservationResult, ObservationContract
    from observations.record.anchors import Anchor, AnchorSet
    from observations.record.integrity import IntegrityRoot
    from observations.record.version import SnapshotVersion
...
class ObservationGroup:
    ...
    limitation: Optional[str] = None  # What this eye cannot see
...
class SnapshotPayload:
    ...
    def from_groups(cls, groups: List[ObservationGroup]) -> 'SnapshotPayload':
        ...
        sorted_groups = sorted(
            groups,
            key=lambda g: (g.category.value, g.eye_name)
        )
    ...
    def get_group(self, category: ObservationCategory, eye_name: str) -> Optional[ObservationGroup]:
...
class Snapshot:
    ...
    anchors: Optional[Tuple['Anchor', ...]] = None  # Will be set after creation
    integrity_root: Optional['IntegrityRoot'] = None  # Will be set after creation
    
    # Cache for deterministic serialization
    _canonical_dict: Optional[Dict[str, Any]] = field(default=None, init=False, repr=False, compare=False)
    _canonical_json: Optional[str] = field(default=None, init=False, repr=False, compare=False)
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.metadata, SnapshotMetadata):
            raise TypeError(f"metadata must be SnapshotMetadata, got {type(self.metadata)}")
        
        # Validate payload
        if not isinstance(self.payload, SnapshotPayload):
            raise TypeError(f"payload must be SnapshotPayload, got {type(self.payload)}")
    ...
    def with_anchors(self, anchors: List['Anchor']) -> 'Snapshot':
        ...
        return Snapshot(
            ...
            anchors=tuple(sorted(anchors, key=lambda a: a.identifier)),  # Sort for determinism
    ...
    def to_json(self, canonical: bool = False, indent: Optional[int] = None) -> str:
        """
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Snapshot':
        ...
        if 'eyes_used' in metadata_dict and isinstance(metadata_dict['eyes_used'], list):
        ...
        if 'integrity' in data:
            if TYPE_CHECKING:
                from observations.record.integrity import IntegrityRoot
            integrity_root = data['integrity']  # Will be properly parsed in integrity.py
...
def create_empty_snapshot(source_path: Union[str, Path]) -> Snapshot:
...
def validate_snapshot_for_storage(snapshot: Snapshot) -> Tuple[bool, Optional[str]]:
    """
    ...
    required_metadata_fields = [
        'snapshot_id', 'created_at', 'source_path', 'observation_count'
    ]
    
    for field in required_metadata_fields:
        if not getattr(snapshot.metadata, field, None):
