...
def compute_hash(data: bytes, algorithm: HashAlgorithm = HashAlgorithm.default()) -> str:
    """
...
def compute_string_hash(text: str, algorithm: HashAlgorithm = HashAlgorithm.default()) -> str:
    """
...
def compute_dict_hash(data: Dict[str, Any], algorithm: HashAlgorithm = HashAlgorithm.default()) -> str:
    """
...
def compute_list_hash(items: List[Any], algorithm: HashAlgorithm = HashAlgorithm.default()) -> str:
    """
...
def combine_hashes(hash1: str, hash2: str, algorithm: HashAlgorithm = HashAlgorithm.default()) -> str:
    """
    ...
    try:
        ...
        raise ValueError(f"Invalid hex string in hash: {e}")
...
class HashTreeNode:
    ...
    data_hash: Optional[str] = None  # Hash of actual data (for leaves)
    ...
    def __post_init__(self) -> None:
        ...
        if self.left is None and self.right is None:
            ...
            expected_hash = combine_hashes(
                self.left.hash_value,
                self.right.hash_value,
                self.algorithm
            )
...
class HashTree:
    ...
    def verify_item(self, item: Any, index: int) -> Tuple[bool, Optional[List[str]]]:
        """
        ...
        for side, sibling_hash in reversed(proof):
            if side == 'left':
                current_hash = combine_hashes(sibling_hash, current_hash, self.algorithm)
            else:
                current_hash = combine_hashes(current_hash, sibling_hash, self.algorithm)
...
def verify_snapshot_integrity(snapshot: 'Snapshot') -> Tuple[bool, Optional[str]]:
    """
...
class CorruptionReport:
    ...
    actual: Optional[str] = None  # Actual value (if applicable)
...
def verify_integrity(snapshot: 'Snapshot') -> Tuple[bool, Optional[str]]:
    """
    Verify integrity of a snapshot.
...
def is_valid_hash(hash_str: str, algorithm: Optional[HashAlgorithm] = None) -> bool:
    """
