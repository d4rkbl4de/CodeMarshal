...
if TYPE_CHECKING:
    ...
    from observations.record.integrity import HashAlgorithm
...
class AnchorType(str, Enum):
    ...
    @classmethod
    def from_path(cls, path: Union[str, Path]) -> 'AnchorType':
        ...
        if path_obj.is_dir():
            ...
            artifact_patterns = [
                r'\.pyc$', r'\.so$', r'\.pyd$', r'__pycache__',
                r'\.egg$', r'\.whl$', r'build/', r'dist/', r'\.git/'
            ]
...
class Anchor:
    ...
    # Context
    original_path: Optional[str] = None  # Path at observation time (for reference only)
    relative_path: Optional[str] = None  # Relative path from source root
    ...
    def __str__(self) -> str:
        ...
        return f"{self.anchor_type.value}:{self.content_fingerprint[:16]} ({path_display})"
...
class AnchorGenerator(ABC):
    ...
    @abstractmethod
    def generate(self, path: Union[str, Path], **kwargs: Any) -> Anchor:
    ...
    def generate(self, path: Union[str, Path], **kwargs: Any) -> Anchor:
        ...
        return Anchor(
            ...
            auxiliary_fingerprints={
                'full_content': full_content_hasher.hexdigest()
            }
        )
        ...
        while len(current_level) > 1:
            ...
            for i in range(0, len(current_level), 2):
                if i + 1 < len(current_level):
                    ...
                    hash_val = hashlib.new(self._algorithm, combined.encode()).hexdigest()
                else:
...
class PythonASTAnchorGenerator(AnchorGenerator):
    ...
    def generate(self, path: Union[str, Path], **kwargs: Any) -> Anchor:
        ...
        try:
            ...
            raise RuntimeError("AST module not available")
    ...
    def _normalize_ast(self, node: Any) -> Any:
        ...
        if isinstance(node, ast.Module):
            ...
            node.name = f"func_{hash(node.name) & 0xFFFF:04x}" if self._normalize_identifiers else node.name
            node.body = [self._normalize_ast(child) for child in node.body]
            ...
            if self._ignore_docstrings and isinstance(node.parent, getattr(ast, 'Expr', type(None))):
...
class AnchorFactory:
    ...
    def create_anchor(
        cls,
        path: Union[str, Path],
        method: Optional[ContentFingerprintMethod] = None,
        **kwargs: Any
    ) -> Anchor:
        """
    ...
    def create_anchors_for_snapshot(
        ...
        """
        anchors: List[Anchor] = []
        base_path = Path(snapshot.source_path)
...
class AnchorSet:
    ...
    def get_anchor(self, identifier: str) -> Optional[Anchor]:
    ...
    def find_anchors_by_path(
        ...
        for anchor in self.anchors:
            if exact:
                if (anchor.original_path == path_str or 
                    anchor.relative_path == path_str):
...
class AnchorComparison:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, '_anchors_a', 
                          frozenset(a.identifier for a in self.set_a.anchors))
        object.__setattr__(self, '_anchors_b', 
                          frozenset(a.identifier for a in self.set_b.anchors))
    ...
    def common_anchors(self) -> FrozenSet[Anchor]:
        ...
        return frozenset(
            a for a in self.set_a.anchors if a.identifier in common_ids
        )
    ...
    def unique_to_a(self) -> FrozenSet[Anchor]:
        ...
        return frozenset(
            a for a in self.set_a.anchors if a.identifier in unique_ids
        )
    ...
    def unique_to_b(self) -> FrozenSet[Anchor]:
        ...
        return frozenset(
            a for a in self.set_b.anchors if a.identifier in unique_ids
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'common_anchors': [a.to_dict() for a in sorted(
                self.common_anchors, key=lambda x: x.identifier
            )],
            'unique_to_a': [a.to_dict() for a in sorted(
                self.unique_to_a, key=lambda x: x.identifier
            )],
            'unique_to_b': [a.to_dict() for a in sorted(
                self.unique_to_b, key=lambda x: x.identifier
            )]
        }
    ...
    def summary(self) -> str:
        ...
        for anchor in self.common_anchors:
            common_by_type[anchor.anchor_type] = common_by_type.get(anchor.anchor_type, 0) + 1
...
def verify_anchor_consistency(anchor: Anchor) -> Tuple[bool, Optional[str]]:
    """
...
AnchorFactory.register_generator(
    ContentFingerprintMethod.FULL_CONTENT_SHA256,
    FileContentAnchorGenerator()
)

AnchorFactory.register_generator(
    ContentFingerprintMethod.LINE_HASH_MERKLE,
    LineHashMerkleAnchorGenerator()
)

AnchorFactory.register_generator(
    ContentFingerprintMethod.STRUCTURAL_SKELETON,
    DirectoryStructureAnchorGenerator()
)

AnchorFactory.register_generator(
    ContentFingerprintMethod.AST_HASH,
    PythonASTAnchorGenerator()
)
...
def get_anchor(storage_path: str, snapshot_id: UUID, anchor_id: str) -> Optional[Anchor]:
    """
