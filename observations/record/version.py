...
class SemanticVersion:
    ...
    def can_upgrade_to(self, target: 'SemanticVersion') -> Tuple[bool, Optional[str]]:
...
class TimestampVersion:
    ...
    def can_upgrade_to(self, target: 'TimestampVersion') -> Tuple[bool, Optional[str]]:
        """Timestamp versions can't be upgraded - they're absolute."""
        return False, "Timestamp versions cannot be upgraded (they're absolute)"


VersionType = Union[SemanticVersion, TimestampVersion]
...
class SnapshotVersion:
    ...
    def can_upgrade_to(self, target: 'SnapshotVersion') -> Tuple[bool, Optional[str]]:
        """Determine if upgrade to target version is possible.
...
class VersionCompatibility:
    ...
    @classmethod
    def is_supported(cls, version: SnapshotVersion) -> Tuple[bool, Optional[str]]:
    ...
    @classmethod
    def validate_upgrade_path(cls, 
                             from_version: SnapshotVersion,
                             to_version: SnapshotVersion) -> Tuple[bool, Optional[str]]:
...
class VersionUpgrader:
    ...
    def upgrade(cls, 
                ...
                target_version: SnapshotVersion) -> Dict[str, Any]:
        ...
        if not can_upgrade:
            raise ValueError(f"Cannot upgrade {current_version} to {target_version}: {reason}")
        
        # Find upgrade path (direct or through intermediates)
        upgrade_path = cls._find_upgrade_path(current_version, target_version)
        if not upgrade_path:
            raise ValueError(f"No upgrade path found from {current_version} to {target_version}")
        ...
        for from_ver, to_ver in upgrade_path:
            ...
            if not upgrade_func:
                raise ValueError(f"No upgrade function registered for {from_ver} → {to_ver}")
    ...
    @classmethod
    def _find_upgrade_path(cls,
                          from_version: SnapshotVersion,
                          to_version: SnapshotVersion) -> Optional[list]:
        ...
        # TODO: Implement multi-step upgrade path finding when needed
        return None
...
def validate_snapshot_version(version_str: str) -> Tuple[bool, Optional[str]]:
...
def check_compatibility(from_version: str, to_version: str) -> Tuple[bool, Optional[str]]:
...
def _create_test_snapshot(version: Optional[str] = None) -> Dict[str, Any]:
...
def get_snapshot_version(storage_path: str, snapshot_id: UUID) -> Optional[SnapshotVersion]:
    """
