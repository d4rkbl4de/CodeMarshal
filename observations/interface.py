...
from core.engine import ObservationInterface, CoordinationRequest, CoordinationResult
import datetime

# Import actual eye implementations
from .eyes import get_eye, observe_with
...
class MinimalObservationInterface(ObservationInterface):
    ...
    def coordinate(self, request: CoordinationRequest) -> CoordinationResult:
        ...
        try:
            ...
            observation_types = request.parameters.get("observation_types", ["file_sight"])
                    ...
                    if eye:
                        ...
                        if obs_type == "boundary_sight":
                            ...
                            if config_path.exists():
                                ...
                                for boundary in config.get('boundaries', []):
                                    ...
                                    boundary_defs.append(BoundaryDefinition(
                                        name=boundary.get('name'),
                                        boundary_type=boundary_type_map.get(boundary_type_str, BoundaryType.PACKAGE),
                        ...
                        if hasattr(result, 'raw_payload') and result.raw_payload:
                            ...
                            if hasattr(result.raw_payload, 'statements'):
                                ...
                                observations.append({
                                    "type": obs_type,
                                    "statements": [stmt.__dict__ for stmt in result.raw_payload.statements]
                                })
                            elif hasattr(result.raw_payload, 'crossings'):
                                # BoundarySight
                                observations.append({
                                    "type": obs_type,
                                    "crossings": [cross.__dict__ for cross in result.raw_payload.crossings]
                                })
    ...
    def observe_directory(self, directory_path: Path, streaming: bool = False, 
        ...
        if hasattr(self, '_last_request'):
            observation_types = self._last_request.parameters.get("observation_types", ["file_sight"])
            is_constitutional = self._last_request.parameters.get("constitutional", False)
            ...
            try:
                ...
                if eye:
                    ...
                    if memory_monitor.should_chunk():
                        ...
                        self._observe_chunked(eye, directory_path, obs_type, observations, boundary_crossings, memory_monitor)
                    else:
                        # Normal observation
                        ...
                        if hasattr(result, 'raw_payload') and result.raw_payload:
                            ...
                            if hasattr(result.raw_payload, 'statements'):
                                ...
                                for stmt in statements:
                                    stmt_dict = {
                                        'type': 'import' if not hasattr(stmt, 'names') else 'from_import',
                                        'module': stmt.module if hasattr(stmt, 'module') else None,
                                        'names': list(stmt.names) if hasattr(stmt, 'names') else [],
                                        'line': stmt.line if hasattr(stmt, 'line') else None
                                    }
                                    stmt_violations = self._boundary_checker.check_import_statement(
                                        directory_path / f"{stmt.module}.py" if hasattr(stmt, 'module') and stmt.module else directory_path,
                                        stmt_dict
                                    )
                                ...
                                observations.append({
                                    "type": obs_type,
                                    "statements": [stmt.__dict__ for stmt in statements],
        ...
        return {
            ...
            "directory_count": len([d for d in directory_path.rglob("*") if d.is_dir()]),
    ...
    def _observe_chunked(self, eye, directory_path: Path, obs_type: str, 
                        observations: List[Dict], boundary_crossings: List[Dict], 
                        memory_monitor) -> None:
        ...
        for i in range(0, len(all_files), chunk_size):
            ...
            if memory_status['emergency_save_triggered']:
                print(f"⚠️ Emergency save triggered during chunked observation", flush=True)
                break
    
    def _observe_directory_streaming(self, directory_path: Path, observation_types: List[str],
                                    session_id: str, memory_monitor) -> Dict[str, Any]:
        """
        ...
        with storage.create_streaming_observation(session_id) as stream:
            ...
            if stream.can_resume_from(session_id):
                ...
                print(f"[RESUME] Restoring from previous session: {resume_info['files_already_processed']} files already processed", flush=True)
            else:
                print(f"[STREAM] Starting fresh observation session", flush=True)
            ...
            if hasattr(stream, 'files_processed') and stream.files_processed > 0:
                ...
                if start_index >= len(all_files):
                    print(f"[COMPLETE] All {len(all_files)} files were already processed", flush=True)
                else:
                    print(f"[RESUME] Continuing from file {start_index + 1} of {len(all_files)}", flush=True)
            
            print(f"[STREAM] Observation: {len(all_files) - start_index} remaining files to process", flush=True)
            ...
            for idx in range(start_index, len(all_files)):
                ...
                if (idx + 1) % 100 == 0:
                    ...
                    print(f"[PROGRESS] {idx + 1}/{len(all_files)} files, "
                    ...
                    if memory_monitor.should_chunk():
                        print(f"[WARNING] Memory pressure detected, continuing with streaming...", flush=True)
            
            # Get final progress
            final_progress = stream.get_progress()
            memory_status = memory_monitor.get_memory_status()
            
            print(f"[COMPLETE] Streaming finished: {final_progress['files_processed']} files, "
                  f"{final_progress['observations_written']} observations, "
                  f"{final_progress['boundary_crossings']} boundary crossings", flush=True)
