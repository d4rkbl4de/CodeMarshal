...
class ParameterInfo:
    ...
    name: str
    annotation: str | None = None  # Raw string representation
    default: str | None = None  # Raw string representation
...
class FunctionSignature:
    ...
    returns: str | None = None  # Return annotation as string
...
class ClassSignature:
    ...
    def magic_methods(self) -> tuple[MethodInfo, ...]:
        ...
        return tuple(
            m for m in self.methods if m.name.startswith("__") and m.name.endswith("__")
        )
...
class ExportDefinition:
    ...
    visibility: Visibility
    signature: FunctionSignature | ClassSignature | None = None
    source_line: int = 0
    source_column: int = 0
...
class ModuleExports:
    ...
    def callable_exports(self) -> tuple[ExportDefinition, ...]:
        ...
        return tuple(
            d
            for d in self.public_definitions
...
class ExportSight(AbstractEye):
    ...
    def _extract_exports(
        self, module_path: Path, content: str
    ...
    def _extract_function_signature(self, node: ast.FunctionDef) -> FunctionSignature:
        ...
        for i, arg in enumerate(args.args):
            param = ParameterInfo(
                name=arg.arg,
                annotation=self._format_annotation(arg.annotation)
                if arg.annotation
                else None,
                default=self._format_default(args.defaults, i, len(args.args))
                if args.defaults
                else None,
        ...
        if args.vararg:
            param = ParameterInfo(
                ...
                annotation=self._format_annotation(args.vararg.annotation)
        ...
        for i, arg in enumerate(args.kwonlyargs):
            param = ParameterInfo(
                ...
                default=self._format_default(args.kw_defaults, i, len(args.kwonlyargs))
                if args.kw_defaults
                else None,
                kind="KEYWORD_ONLY",
            )
        ...
        if args.kwarg:
            param = ParameterInfo(
                name=args.kwarg.arg,
                annotation=self._format_annotation(args.kwarg.annotation)
    ...
    def _extract_class_definition(self, node: ast.ClassDef) -> ExportDefinition | None:
    ...
    def _format_annotation(self, node: ast.AST) -> str | None:
    ...
    def _format_default(
        self, defaults: list[ast.AST], index: int, total: int
    ) -> str | None:
...
def observe_exports(file_path: str | Path) -> ModuleExports:
