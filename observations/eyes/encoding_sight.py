...
class LineEndingStats:
    ...
    def is_consistent(self) -> bool:
        ...
        non_zero = sum(
            1 for count in [self.lf_count, self.crlf_count, self.cr_count] if count > 0
        )
        return non_zero <= 1
...
class EncodingSight(AbstractEye):
    ...
    def _observe_impl(self, target: Path) -> ObservationResult:
        ...
        try:
            ...
            with open(target, "rb") as f:
                ...
                observation = self._analyze_file(
                    target, file_size, sample, f, timestamp
                )
    ...
    def _analyze_file(
        ...
        has_control_chars = (
            encoding_metrics.control_char_count > 10
        )  # Arbitrary threshold
    ...
    def _detect_encoding(
        self, sample: bytes, bom_type: BOMType, file_path: Path
    ...
    def validate(self) -> bool:
        ...
        write_operations = {
            ...
            "open(" + '"a")',
            ".trun" + "cate(",
            ".se" + "ek(",  # Seeking is allowed for read, not write
        }
...
def detect_encoding(file_path: str | Path) -> tuple[str, EncodingConfidence]:
...
def check_line_endings(file_path: str | Path) -> LineEndingType:
