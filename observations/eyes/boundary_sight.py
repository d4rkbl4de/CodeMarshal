...
from .import_sight import ImportObservation, ImportSight, ImportStatement
...
class ModuleNode:
    ...
    package_path: Path | None = None  # Directory containing __init__.py
...
class DependencyEdge:
    ...
    def is_external_dependency(self) -> bool:
        ...
        return "." in self.target_module and not self.target_module.startswith(
            self.source_module.split(".")[0]
        )
...
class BoundaryObservation:
    ...
    module_boundaries: tuple[tuple[str, str], ...] = field(
        default_factory=tuple
    )  # module -> boundary
...
class BoundarySight(AbstractEye):
    ...
    def __init__(
        self,
        boundary_definitions: list[BoundaryDefinition] | None = None,
        ...
        for file_path in python_files:
            ...
            try:
                ...
                for stmt in import_obs.statements:
                    ...
                    target_module = self._resolve_import(stmt, source_module, modules)
    ...
    def _resolve_import(
        self, stmt: ImportStatement, source_module: str, modules: dict[str, ModuleNode]
    ) -> str | None:
    ...
    def _assign_boundaries(self, modules: dict[str, ModuleNode]) -> dict[str, str]:
    ...
    def _detect_boundary_crossings(
        ...
        for edge in dependencies:
            ...
            if source_boundary == target_boundary or "UNKNOWN" in (
                source_boundary,
                target_boundary,
            ):
                continue
                ...
                for bd in self.boundary_definitions:
                    if bd.name == source_boundary:
                        ...
                        for pattern in bd.allowed_targets:
                            ...
                            if fnmatch.fnmatch(
                                target_normalized, pattern
                            ) or fnmatch.fnmatch(target_boundary, pattern):
                                allowed = True
                                break
                        break
        ...
        for source, target in circular_deps:
            ...
            for crossing in crossings:
                if (
                    crossing.source_module == source
                    and crossing.target_module == target
                ):
    ...
    def _calculate_confidence(
        self, observation: BoundaryObservation, file_count: int
    ) -> float:
        ...
        if total_deps > 0:
            unresolved = sum(
                1
                for d in observation.dependencies
                if d.target_module not in observation.modules
            )
...
def create_layer_boundary(
    name: str, pattern: str, allowed_targets: list[str] = None
) -> BoundaryDefinition:
...
def create_package_boundary(
    package_name: str, allowed_targets: list[str] = None
) -> BoundaryDefinition:
...
def observe_boundaries(
    root_path: str | Path, boundaries: list[BoundaryDefinition] | None = None
) -> BoundaryObservation:
