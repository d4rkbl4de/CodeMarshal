...
class PermissionSet(NamedTuple):
    ...
    def octal_string(self) -> str:
        ...
        octet = 0
        if self.user_read: octet |= 0o400
        if self.user_write: octet |= 0o200
        if self.user_execute: octet |= 0o100
        if self.group_read: octet |= 0o040
        if self.group_write: octet |= 0o020
        if self.group_execute: octet |= 0o010
        if self.other_read: octet |= 0o004
        if self.other_write: octet |= 0o002
        if self.other_execute: octet |= 0o001
...
class FileMetadata:
    ...
    path: PurePath
    resolved_path: Optional[PurePath] = None  # If symlink resolved
...
class FileSight(AbstractEye):
    ...
    def __init__(self, config: Optional[TraversalConfig] = None) -> None:
    ...
    def observe(self, target: Union[str, Path]) -> ObservationResult:
    ...
    def _observe_impl(self, target: Union[str, Path]) -> ObservationResult:
        """
    ...
    def _observe_single_file(self, path: Path, timestamp: datetime) -> DirectoryTree:
        ...
        tree = DirectoryTree(
            ...
            directories=(),
            symlinks=(),
            errors=((PurePath(path.name), metadata.observation_errors),) if metadata and metadata.observation_errors else ()
        )
    ...
    def _collect_file_metadata(self, path: Path, root: Path) -> Optional[FileMetadata]:
        """
        ...
        # Basic path info
        metadata['path'] = PurePath(path.relative_to(root) if path.is_relative_to(root) else path)
        ...
        try:
            ...
            metadata['access_time'] = datetime.fromtimestamp(
            ...
            if (metadata['file_type'] == FileType.FILE and 
                self.config.compute_hashes and
                metadata['size_bytes'] <= self.config.hash_limit_mb * 1024 * 1024):
                try:
    ...
    def _compute_file_hash(self, path: Path) -> str:
        ...
        try:
            ...
            raise OSError(f"Cannot read file for hashing: {str(e)}")
    ...
    def validate(self) -> bool:
        ...
        prohibited_imports = {
            ...
            'os.sys' + 'tem', 'os.po' + 'pen',  # Shell commands
            'import' + 'lib', 'run' + 'py',  # Code loading
        }
...
def observe_file(file_path: Union[str, Path]) -> Optional[FileMetadata]:
