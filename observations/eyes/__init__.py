...
from typing import (
    Dict, List, Optional, Tuple, Union, Type, Any
)
...
from .base import (
    Eye, ObservationResult, ObservationError, ErrorContext,
    CompositeObservation, validate_eye_purity
)

# Import all concrete eyes
from .file_sight import (
    FileSight, observe_file, observe_directory,
    FileMetadata, DirectoryTree, TraversalConfig,
    FileType, PermissionSet
)

from .import_sight import (
    ImportSight, observe_imports,
    ImportStatement, ImportObservation
)

from .export_sight import (
    ExportSight, observe_exports,
    ModuleExports, ExportDefinition, Visibility, DefinitionType,
    FunctionSignature, ClassSignature, ParameterInfo
)

from .boundary_sight import (
    BoundarySight, observe_boundaries,
    BoundaryObservation, BoundaryDefinition, BoundaryType, BoundarySeverity,
    ModuleNode, DependencyEdge, BoundaryCrossing,
    create_layer_boundary, create_package_boundary
)

from .encoding_sight import (
    EncodingSight, detect_encoding, check_line_endings,
    EncodingObservation, EncodingConfidence, LineEndingType,
    BOMType, EncodingMetrics, LineEndingStats
)
...
class EyeRegistry:
    ...
    def register_eye(self, eye_class: Type[Eye], name: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            warnings.warn(f"Cannot instantiate {eye_class.__name__}: {e}")
            return False
        ...
        if eye_name in self._registry:
            warnings.warn(f"Eye '{eye_name}' already registered")
            return False
    ...
    def get_eye(self, name: str) -> Optional[Eye]:
        """
        ...
        if not entry.validated:
            warnings.warn(f"Eye '{name}' failed purity validation")
            return None
        
        return entry.get_or_create_instance()
    
    def get_eye_class(self, name: str) -> Optional[Type[Eye]]:
    ...
    def observe_with_all(
        self, 
        target: Path,
        eye_names: Optional[List[str]] = None
...
def get_eye(name: str) -> Optional[Eye]:
...
def observe_with(
    ...
    **kwargs: Any
    ...
    try:
        ...
        warnings.warn(f"Observation failed with {eye_name}: {e}")
        return None
...
def observe_python_file(file_path: Union[str, Path]) -> CompositeObservation:
    """
...
def observe_python_directory(dir_path: Union[str, Path]) -> CompositeObservation:
    """
...
def export_observations_to_dict(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for obs in composite_dir.observations:
            if obs.provenance and obs.provenance.observer_name == "boundary_sight":
                if obs.raw_payload:
                    print(f"   Boundary analysis completed")
