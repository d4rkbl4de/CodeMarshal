...
class SnapshotLimitations:
    ...
    declaration_hash: str
    timestamp: Optional[str]  # ISO format when set
...
def validate_limitations() -> list[str]:
    ...
    for doc_id in documented_ids - declared_ids:
        errors.append(
            f"Documentation exists for non-declared limitation: '{doc_id}'"
        )
...
def validate_during_snapshot(snapshot_limitations: SnapshotLimitations) -> None:
    ...
    if snapshot_ids == current_ids and snapshot_limitations.declaration_hash != current_set.declaration_hash:
        errors.append(
