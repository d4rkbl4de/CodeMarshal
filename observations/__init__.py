...
from .eyes import (
    ...
    CompositeObservation, validate_eye_purity, validate_all_eyes,
    
    # Concrete eye classes
    FileSight, ImportSight, ExportSight, BoundarySight, EncodingSight,
    
    # Common functions
    get_eye, list_eyes, get_capabilities,
    observe_with, observe_comprehensive,
    observe_python_file, observe_python_directory,
    ...
    BoundaryObservation, BoundaryDefinition, BoundaryType,
...
class ObservationSystem:
    ...
    def observe(
        ...
        **kwargs: Any
        ...
        if not eye:
            ...
            raise ValueError(
                f"Eye '{eye_name}' not found or invalid. "
                f"Available eyes: {available}"
            )
        ...
        if not validation_result.is_valid:
            raise ValueError(
                f"Cannot observe target with {eye_name}: "
    ...
    def create_snapshot(
        ...
        description: str = "",
        include_eyes: Optional[List[str]] = None,
        exclude_eyes: Optional[List[str]] = None
    ) -> Snapshot:
        """
...
def _initialize() -> None:
    ...
    try:
        ...
        if not eyes:
            warnings.warn("No observation eyes available", RuntimeWarning)
    except Exception as e:
        warnings.warn(f"Failed to get available eyes: {e}", RuntimeWarning)
