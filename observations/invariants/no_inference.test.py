...
class InferenceDetector:
    ...
    def check_for_inference(self, text: str, context: str = "") -> list[InferenceCheck]:
        ...
        for pattern in self.guessing_patterns:
            if pattern.search(text):
                checks.append(
                    (True, f"Guessing language in {context}: '{pattern.pattern}'")
                )
        ...
        for pattern in self.intent_patterns:
            if pattern.search(text):
                checks.append(
                    (True, f"Intent inference in {context}: '{pattern.pattern}'")
                )
        ...
        for pattern in self.interpretation_patterns:
            if pattern.search(text):
                checks.append(
                    (True, f"Semantic interpretation in {context}: '{pattern.pattern}'")
                )
        ...
        for pattern in self.confidence_patterns:
            if pattern.search(text):
                checks.append(
                    (True, f"Statistical confidence in {context}: '{pattern.pattern}'")
                )
    ...
    def check_uncertainty_markers(
        ...
        if has_uncertainty_term:
            ...
            has_marker = any(
                pattern.search(text) for pattern in self.uncertainty_markers
            )
            if not has_marker:
                checks.append(
                    (True, f"Uncertainty without marker in {context}: '{text}'")
                )

        return checks

    def check_pattern_output(
        self, pattern_output: PatternOutput, pattern_name: str
    ) -> list[InferenceCheck]:
        """
        ...
        checks.extend(
            self.check_uncertainty_markers(output_str, f"pattern {pattern_name}")
        )
        ...
        if pattern_name.lower() in ["density", "coupling", "complexity"]:
            ...
            for key, value in pattern_output.items():
                if isinstance(value, str):
                    ...
                    checks.extend(
                        self.check_for_inference(
                            value, f"pattern {pattern_name} key {key}"
                        )
                    )

        return checks

    def check_observation_output(
        self, observation_output: ObservationOutput, eye_name: str
    ) -> list[InferenceCheck]:
        """
        ...
        any(pattern.search(output_str) for pattern in self.limitation_patterns)
...
class TestNoInference:
    ...
    def test_file_sight_no_inference(self) -> None:
        ...
        for obs in observations:
            checks = self.detector.check_observation_output(
                obs.__dict__ if hasattr(obs, "__dict__") else obs, "FileSight"
            )
    ...
    def test_import_sight_no_inference(self) -> None:
        ...
        for obs in observations:
            checks = self.detector.check_observation_output(
                obs.__dict__ if hasattr(obs, "__dict__") else obs, "ImportSight"
            )
            for has_inference, evidence in checks:
                if has_inference:
                    violations.append(evidence)
        ...
        assert len(violations) == 0, "ImportSight inference detected:\n" + "\n".join(
    ...
    def test_export_sight_no_inference(self) -> None:
        ...
        for obs in observations:
            checks = self.detector.check_observation_output(
                obs.__dict__ if hasattr(obs, "__dict__") else obs, "ExportSight"
            )
            for has_inference, evidence in checks:
                if has_inference:
                    violations.append(evidence)
        ...
        assert len(violations) == 0, "ExportSight inference detected:\n" + "\n".join(
            f"  • {v}" for v in violations
        )
    ...
    def test_boundary_sight_no_inference(self) -> None:
        ...
        for obs in observations:
            checks = self.detector.check_observation_output(
                obs.__dict__ if hasattr(obs, "__dict__") else obs, "BoundarySight"
            )
            for has_inference, evidence in checks:
                if has_inference:
                    violations.append(evidence)
        ...
        assert len(violations) == 0, "BoundarySight inference detected:\n" + "\n".join(
            f"  • {v}" for v in violations
        )
    ...
    def test_encoding_sight_no_inference(self) -> None:
        ...
        for obs in observations:
            checks = self.detector.check_observation_output(
                obs.__dict__ if hasattr(obs, "__dict__") else obs, "EncodingSight"
            )
            for has_inference, evidence in checks:
                if has_inference:
                    violations.append(evidence)
        ...
        assert len(violations) == 0, "EncodingSight inference detected:\n" + "\n".join(
            f"  • {v}" for v in violations
        )
    ...
    def test_density_analyzer_no_inference(self) -> None:
        ...
        checks = self.detector.check_pattern_output(
            density_output.__dict__
            if hasattr(density_output, "__dict__")
            else density_output,
            "DensityAnalyzer",
        )
        ...
        assert len(violations) == 0, (
            "DensityAnalyzer inference detected:\n"
            + "\n".join(f"  • {v}" for v in violations)
        )
    ...
    def test_coupling_analyzer_no_inference(self) -> None:
        ...
        checks = self.detector.check_pattern_output(
            coupling_output.__dict__
            if hasattr(coupling_output, "__dict__")
            else coupling_output,
            "CouplingAnalyzer",
        )
        ...
        assert len(violations) == 0, (
            "CouplingAnalyzer inference detected:\n"
            + "\n".join(f"  • {v}" for v in violations)
        )
    ...
    def test_complexity_analyzer_no_inference(self) -> None:
        ...
        checks = self.detector.check_pattern_output(
            complexity_output.__dict__
            if hasattr(complexity_output, "__dict__")
            else complexity_output,
            "ComplexityAnalyzer",
        )
        ...
        assert len(violations) == 0, (
            "ComplexityAnalyzer inference detected:\n"
            + "\n".join(f"  • {v}" for v in violations)
        )
    ...
    def test_boundary_violation_detector_no_inference(self) -> None:
        ...
        checks = self.detector.check_pattern_output(
            violation_output.__dict__
            if hasattr(violation_output, "__dict__")
            else violation_output,
        ...
        assert len(violations) == 0, (
            "BoundaryViolationDetector inference detected:\n"
            + "\n".join(f"  • {v}" for v in violations)
        )
    ...
    def test_uncertainty_quantifier_explicit_markers(self) -> None:
        ...
        output_str = json.dumps(
            uncertainty_output.__dict__
            if hasattr(uncertainty_output, "__dict__")
            else uncertainty_output,
            indent=2,
        )

        # UncertaintyQuantifier MUST use explicit markers
        has_marker = any(
            pattern.search(output_str) for pattern in self.detector.uncertainty_markers
        )
        ...
        assert len(violations) == 0, (
            "UncertaintyQuantifier issues detected:\n"
            + "\n".join(f"  • {v}" for v in violations)
        )
    ...
    def test_observation_validation_no_inference(self) -> None:
        ...
        assert len(violations) == 0, (
            "Observation validation inference detected:\n"
            + "\n".join(f"  • {v}" for v in violations)
        )
...
''')
        ...
        for eye_name, eye in eyes:
            try:
                ...
                for obs in observations:
                    checks = self.detector.check_observation_output(
                        obs.__dict__ if hasattr(obs, "__dict__") else obs, eye_name
                    )
                    for has_inference, evidence in checks:
                        if has_inference:
