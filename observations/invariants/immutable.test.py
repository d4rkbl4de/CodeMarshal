...
from observations.record.integrity import HashMismatchError, compute_hash, verify_hash

# System imports
from observations.record.snapshot import Snapshot, create_snapshot, get_snapshot_anchor
from observations.record.version import (
    create_new_version,
    get_snapshot_version,
)
from storage.atomic import AtomicWriteError, atomic_write
from storage.corruption import CorruptionError, detect_corruption
...
class ImmutabilityAssassin:
    ...
    def attempt_pickle_attack(self, obj: Any, obj_name: str) -> list[str]:
        ...
        try:
            ...
            # that's a violation
            unpickled_success = self.attempt_object_mutation(
                unpickled, f"unpickled_{obj_name}"
            )
    ...
    def attempt_version_manipulation(self, snapshot: Snapshot) -> list[str]:
        ...
        version_success = self.attempt_object_mutation(
            original_version, "SnapshotVersion"
        )
        if version_success:
        ...
        try:
            ...
            if (
                get_snapshot_version(new_version).version_id
    ...
    def attempt_anchor_manipulation(self, snapshot: Snapshot) -> list[str]:
        ...
        try:
            ...
            hashlib.sha256(mutated_json.encode()).hexdigest()
    ...
    def test_concurrent_write_protection(self) -> list[str]:
        ...
        try:
            ...
            if final_content not in [attempt1_data, attempt2_data, test_data]:
                violations.append(
                    f"Concurrent write produced corrupted data: {final_content[:50]}"
                )
...
class TestImmutableObservations:
    ...
    def setup_method(self) -> None:
        ...
        self.snapshot = create_snapshot(
            self.file_observations + self.import_observations
        )
...
""")
    ...
    def test_snapshot_immutable(self) -> None:
        ...
        snapshot_mutations = self.assassin.attempt_object_mutation(
        ...
        pickle_mutations = self.assassin.attempt_pickle_attack(
            self.snapshot, "Snapshot"
        )
        if pickle_mutations:
        ...
        # Tier-1 failure
        assert len(violations) == 0, "Snapshot mutability detected:\n" + "\n".join(
    ...
    def test_snapshot_versioning(self) -> None:
        ...
        # Tier-1 failure
        assert len(violations) == 0, (
            "Snapshot versioning violation detected:\n"
            + "\n".join(f"  • {v}" for v in violations)
        )
    ...
    def test_hash_integrity(self) -> None:
        ...
        try:
            snapshot_dict = (
                ...
                if hasattr(self.snapshot, "to_dict")
                else self.snapshot.__dict__
            )
            snapshot_json = json.dumps(snapshot_dict, sort_keys=True)
            snapshot_bytes = snapshot_json.encode()

            # Test hash collision attempts
            collisions = self.assassin.attempt_hash_collision(
            ...
            hash_violations = self.corruption_detector.test_hash_verification(
                self.snapshot
            )
            if hash_violations:
        ...
        # Tier-1 failure
        assert len(violations) == 0, "Hash integrity violation detected:\n" + "\n".join(
            f"  • {v}" for v in violations
        )
    ...
    def test_atomic_write_guarantees(self) -> None:
        ...
        # Tier-1 failure
        assert len(violations) == 0, "Atomic write violation detected:\n" + "\n".join(
            f"  • {v}" for v in violations
        )
    ...
    def test_corruption_detection(self) -> None:
        ...
        corruption_violations = self.corruption_detector.test_corruption_detection(
            test_data
        )
        ...
        # Tier-1 failure
        assert len(violations) == 0, (
            "Corruption detection violation detected:\n"
            + "\n".join(f"  • {v}" for v in violations)
        )
    ...
    def test_retroactive_alteration_prevention(self) -> None:
        ...
        # Tier-1 failure
        assert len(violations) == 0, (
            "Retroactive alteration violation detected:\n"
            + "\n".join(f"  • {v}" for v in violations)
        )
    ...
    def test_hostile_immutability_attack(self) -> None:
        ...
        if all_violations:
            ...
            with open(report_path, "w") as f:
                ...
                f.write(
                    f"Snapshot version: {get_snapshot_version(self.snapshot).version_id}\n"
                )
    ...
    def _test_deep_nesting_mutation(self) -> None:
            ...
            "observations": [
                {
                    ...
                    "content_hash": "abc123",
                    "metrics": {"lines": 10, "imports": ["typing", "os"], "depth": 2},
                }
            ],
            "patterns": {
                "density": 0.5,
        ...
        assert len(violations) == 0, "Deep nesting mutation detected:\n" + "\n".join(
            f"  • {v}" for v in violations
        )
        ...
        for obs in observations:
            ...
            for field in inferred_fields:
                if field in str(obs_dict).lower():
                    pytest.fail(
                        f"Article 1 violation: Contains inferred field '{field}'"
                    )
