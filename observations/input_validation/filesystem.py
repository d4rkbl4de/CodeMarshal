            ...
            try:
                ...
                with os.scandir(canonical_path) as entries:
                    for entry in entries:
                        ...
                        # Full validation
                        validation = self.validate_path(entry_path, require_exists=False)
                        if validation["valid"]:
            ...
            if current.is_symlink():
                ...
                try:
                    ...
                    raise SymlinkViolationError(
                        f"Cannot read symlink {current}: {e}"
                    )
...
def _test_module() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if sys.platform != "win32":  # Symlink tests don't work well on Windows
            ...
            assert result["valid"] is False, "Symlinks should be rejected in strict mode"
