...
def validate_filesystem_access(path: str | Path) -> bool:
    """Check if filesystem path is accessible."""
    path = Path(path)
    return path.exists()


def validate_binary_file(path: str | Path) -> bool:
    """
...
def validate_size_limits(path: str | Path, limit_mb: int = 100) -> bool:
...
def is_safe_to_observe(path: str | Path) -> bool:
    """
