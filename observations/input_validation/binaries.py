...
class BinaryDetectionPolicy:
    ...
    known_binary_extensions: Set[str] = field(default_factory=lambda: {
        '.exe', '.dll', '.so', '.dylib', '.bin', '.o', '.obj', '.class',
        '.pyc', '.pyo', '.pyd', '.jar', '.war', '.ear', '.zip', '.tar',
        '.gz', '.bz2', '.xz', '.rar', '.7z', '.iso', '.img',
    ...
    known_text_extensions: Set[str] = field(default_factory=lambda: {
        '.py', '.txt', '.md', '.rst', '.json', '.yaml', '.yml', '.toml',
        '.ini', '.cfg', '.conf', '.xml', '.html', '.htm', '.css', '.js',
        '.ts', '.rs', '.go', '.java', '.cpp', '.c', '.h', '.hpp',
        '.cs', '.rb', '.php', '.pl', '.sh', '.bash', '.zsh',
        '.csv', '.tsv', '.log',
    })
    
    # MIME types that are definitely binary (if mimetypes detection is available)
    binary_mime_types: Set[str] = field(default_factory=lambda: {
...
class BinaryValidator:
    ...
    def __init__(
        self,
        policy: Optional[BinaryDetectionPolicy] = None
    ) -> None:
        """
    ...
    def classify_file(
        ...
        if file_size is None:
            try:
                ...
                raise OSError(f"Cannot stat file {resolved_path}: {e}")
        ...
        if binary_indicators:
            ...
            result["reason"] = f"Binary indicators detected: {', '.join(binary_indicators)}"
            return result
        ...
        if encoding_result["success"]:
            ...
            if extension_hint == "text":
                ...
                result["reason"] += f" (but has binary extension .{resolved_path.suffix})"
        else:
    ...
    def _check_extension(self, file_path: Path) -> Optional[str]:
        """
        ...
        if mime_type:
            if any(mime_type.startswith(binary_prefix) 
    ...
    def _try_decode_as_text(self, data: bytes) -> Dict[str, Any]:
        ...
        for encoding in self._policy.text_encodings:
            try:
                ...
                decoded = data.decode(encoding, errors='strict' if self._policy.strict_decoding else 'replace')
    ...
    def is_definitely_binary(self, classification_result: Dict[str, Any]) -> bool:
        ...
        if (classification == BinaryClassification.UNKNOWN and 
            confidence >= 0.9 and 
            classification_result.get("binary_indicators")):
            return True
