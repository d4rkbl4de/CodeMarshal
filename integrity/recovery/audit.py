...
from typing import Dict, Any, Optional, List, Tuple, NamedTuple, Union
import logging

# Core imports - truth preservation layers
from storage.atomic import atomic_write, atomic_read
from storage.corruption import detect_corruption
from integrity.monitoring.errors import log_error, ErrorSeverity, ErrorCategory
...
class AuditEvent(NamedTuple):
    ...
    signature_hash: Optional[str] = None  # Hash of this event for next link
...
def create_audit_directory(audit_root: Optional[Path] = None) -> Path:
    """
...
def generate_event_id(action: str, component: str) -> str:
    ...
    return f"{EVENT_ID_PREFIX}{timestamp}_{component_short}_{action_short}_{unique_suffix}"


def get_latest_audit_hash(audit_file: Path) -> Optional[str]:
    """
...
def log_audit_event(action: str, 
                    component: str, 
                    metadata: Dict[str, Any],
                    audit_root: Optional[Path] = None) -> AuditEvent:
    """
    ...
    try:
        ...
        log_error(error_msg, severity=ErrorSeverity.CRITICAL, category=ErrorCategory.AUDIT_FAILURE)
    ...
    try:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            try:
                ...
                # Basic validation
                required_fields = ["event_id", "timestamp", "action", "component", "metadata", "signature_hash"]
...
def query_audit_events(component: Optional[str] = None,
                       action: Optional[str] = None,
                       start_time: Optional[datetime] = None,
                       end_time: Optional[datetime] = None,
                       audit_root: Optional[Path] = None) -> List[AuditEvent]:
    """
    ...
    if component:
        ...
        search_dirs = [root_dir / d for d in ["recovery", "observations", "inquiry", "lens", "system"]]
...
def create_audit_summary(audit_root: Optional[Path] = None) -> AuditSummary:
    """
...
def export_audit_trail(format: str = "json",
                       output_path: Optional[Path] = None,
                       audit_root: Optional[Path] = None) -> Path:
    """
    ...
    if format.lower() == "json":
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(["event_id", "timestamp", "component", "action", "metadata", "previous_hash", "signature_hash"])
...
def cleanup_old_audit_logs(audit_root: Optional[Path] = None) -> Dict[str, int]:
    """
    ...
    """
    root_dir = create_audit_directory(audit_root)
    cutoff_time = datetime.now(timezone.utc).timestamp() - (AUDIT_RETENTION_DAYS * 24 * 60 * 60)
            ...
            if audit_file.stat().st_mtime < cutoff_time:
                ...
                log_audit_event(
                    ...
                    metadata={
                        ...
                        "cutoff_time": datetime.fromtimestamp(cutoff_time, tz=timezone.utc).isoformat()
                    },
                    audit_root=root_dir
                )
    ...
    for dirpath in reversed(list(root_dir.rglob("*"))):  # Reverse to delete deepest first
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Manage and verify CodeMarshal audit logs")
    ...
    verify_parser.add_argument("--all", action="store_true", help="Verify all audit files")
    ...
    if args.command == "verify":
        if args.file:
            ...
            if valid:
                ...
                for issue in issues:
                    print(f"  - Line {issue.get('line', '?')}: {issue.get('error', 'Unknown')}")
        elif args.all:
        ...
        print(f"  Chain integrity: {'✓ Valid' if summary.valid_chain else f'✗ {summary.broken_links} broken links'}")
