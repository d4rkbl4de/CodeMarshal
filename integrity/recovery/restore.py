...
def validate_backup_file(backup_path: Path) -> Tuple[bool, Optional[str], Optional[Dict[str, Any]]]:
    """
    ...
    if computed_hash != expected_hash:
        return False, f"Integrity hash mismatch: expected {expected_hash[:16]}..., got {computed_hash[:16]}...", None
...
def create_restoration_checkpoint(backup_path: Path) -> Optional[Path]:
    """
    ...
    try:
        ...
        checkpoint_data = {
            ...
            "state": current_state.to_dict() if hasattr(current_state, 'to_dict') else dict(current_state),
            "snapshot": current_snapshot.to_dict() if hasattr(current_snapshot, 'to_dict') else dict(current_snapshot),
...
def perform_restore(backup_file_path: str) -> RestorationOutcome:
    ...
    try:
        ...
        if snapshot_restored and checkpoint_path:
            try:
                ...
                log_error(f"State restoration failed, checkpoint available at {checkpoint_path}")
    ...
    restoration_end = datetime.now(timezone.utc)
...
def list_available_backups(backup_dir: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
    """
...
def get_backup_info(backup_file_path: str) -> Optional[Dict[str, Any]]:
    """
    ...
    return {
        ...
        "backup_age_days": (
            ...
            datetime.fromisoformat(metadata["timestamp"]).replace(tzinfo=timezone.utc)
        ).days
    }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Restore CodeMarshal state from backup")
    parser.add_argument("backup_file", help="Path to backup JSON file")
    parser.add_argument("--dry-run", action="store_true", help="Validate but don't restore")
