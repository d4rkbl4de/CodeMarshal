...
def run_full_recovery_cycle(
    backup_dir: str | None = None, audit_dir: str | None = None
    ...
    if backup_outcome.success and backup_outcome.backup_path:
        ...
        audit_event = audit_recovery(
            ...
            metadata={
                ...
                "manifest": backup_outcome.manifest._asdict()
                if backup_outcome.manifest
                else None,
            },
        )
...
def validate_recovery_system(
    test_backup: bool = True, test_restore: bool = True, test_audit: bool = True
) -> dict[str, Any]:
    """
...
def get_recovery_status(
    ...
    try:
        ...
        status["components"]["backup"] = {"available": False, "error": str(e)}
    ...
    try:
        ...
        status["components"]["audit"] = {"available": False, "error": str(e)}
    ...
    elif not status.get("components", {}).get("audit", {}).get("chain_integrity", True):
...
def _initialize_recovery_system() -> None:
    ...
    try:
        ...
        log_error(
            f"Recovery system initialization failed: {e}", severity=ErrorSeverity.MEDIUM
        )
