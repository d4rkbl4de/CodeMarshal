...
class MutationDetector:
    ...
    def check_source_for_mutation(self, module_name: str, source_code: str) -> List[Dict[str, str]]:
        """
    ...
    def check_object_mutation(self, obj: Any, obj_name: str, path: str = "") -> List[Dict[str, str]]:
        """
                ...
                if hasattr(original_copy, '__dict__'):
                    ...
                    for attr in added:
                        violations.append({
                            ...
                            "target": f"{obj_name}.{attr}" if path else f"{obj_name}",
    ...
    def check_mutable_defaults(self, module_name: str, source_code: str) -> List[Dict[str, str]]:
        """
...
class MutationASTVisitor(ast.NodeVisitor):
    ...
    MUTATING_METHODS: Set[str] = {
        ...
        'clear', 'copy',  # copy doesn't mutate, but included for completeness
    }
            ...
            if isinstance(target, ast.Attribute):
                ...
                if self._is_protected_attribute(attr_name, node.value):
                    self._record_violation(
                        ...
                        target=ast.unparse(target) if hasattr(ast, 'unparse') else str(target)
                    )
        ...
        if isinstance(node.func, ast.Attribute):
            ...
            if method_name in self.MUTATING_METHODS:
                ...
                if isinstance(node.func.value, (ast.Name, ast.Attribute)):
                    ...
                    obj_expr = ast.unparse(node.func.value) if hasattr(ast, 'unparse') else str(node.func.value)
                    
                    # Simple heuristic: check if object name suggests observation/state
                    if any(keyword in obj_expr.lower() for keyword in 
                           ['observation', 'snapshot', 'state', 'context', 'record']):
                        self._record_violation(
    ...
    def _is_protected_attribute(self, attr_name: str, value: Optional[ast.AST]) -> bool:
        """
        ...
        """
        protected_keywords = ['_observation', '_state', '_context', '_snapshot',
                             '_record', '_data', '_immutable', 'observations',
                             'state', 'context']
...
def enforce_no_mutation() -> List[Dict[str, str]]:
        ...
        try:
            ...
            if source_file:
                with open(source_file, 'r', encoding='utf-8') as f:
                    source = f.read()
        except (OSError, TypeError, IOError, UnicodeDecodeError):
            continue
            
        if source:
            # Check for mutation patterns
            mutation_violations = detector.check_source_for_mutation(module_name, source)
    ...
    if SNAPSHOT_AVAILABLE and load_snapshot:
        try:
            ...
            if snapshot:
                snapshot_violations = detector.check_object_mutation(snapshot, "snapshot")
...
if __name__ == "__main__":
    ...
    if args.install_guard:
        ...
        print(f"Mutation guard installed. Guarding {len(guard._guarded_objects)} objects.")
            ...
            if any(module_name.startswith(prefix) for prefix in 
                ...
                try:
                    ...
                    if source_file:
                        with open(source_file, 'r', encoding='utf-8') as f:
                            source = f.read()
                            defaults_violations = detector.check_mutable_defaults(module_name, source)
