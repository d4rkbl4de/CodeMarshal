...
class NetworkModuleDetector:
    ...
    NETWORK_MODULES: list[str] = [
        ...
        "selectors",
        "select",
        "subprocess",  # subprocess can create network processes
        "ipaddress",
        "email",
        "mailbox",
        "mimetypes",  # email/network related
        ...
        "unittest.mock.patch",
        "unittest.mock.Mock",  # Can mock network calls
    ...
    def check_loaded_modules(self) -> list[dict[str, str]]:
        ...
        for module_name, module in sys.modules.items():
            ...
            module_violations = self._check_module_for_network_imports(
                module, module_name
            )
    ...
    def _check_module_for_network_imports(
    ...
    def _is_network_module(self, module_name: str) -> bool:
        ...
        for network_module in self.NETWORK_MODULES:
            if network_module in module_name_lower or module_name_lower.startswith(
                network_module + "."
            ):
                return True
...
class NetworkAccessGuard:
    ...
    def __init__(self) -> None:
        self._original_modules: dict[str, ModuleType | None] = {}
        self._original_functions: dict[str, Callable | None] = {}
    ...
    def _patch_http_modules(self) -> None:
        ...
        for module_name in http_modules:
            try:
                ...
                def create_guarded_module(mod_name: str):
                    class GuardedModule:
                        def __getattr__(self, name: str) -> Any:
                            raise NetworkAccessViolation(
                                {
                                    "violation": "network_access",
                                    "module": mod_name,
                                    "details": f"Access to {mod_name}.{name} blocked",
                                    "timestamp": datetime.now(UTC).isoformat(),
                                }
                            )
                ...
                sys.modules[module_name] = create_guarded_module(module_name)
...
if __name__ == "__main__":
    ...
    if args.install_guard:
        ...
        print(
            f"Network guard installed. {len(guard._patched_modules)} modules patched."
        )
