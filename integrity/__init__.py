...
try:
    from .validation.observations_test import (
        validate_observations as real_validate_observations,
    )
...
try:
    from .validation.integration_test import (
        validate_integration as real_validate_integration,
    )
...
try:
    from .prohibitions import MODULE_AVAILABILITY as PROHIBITIONS_AVAILABILITY
...
def run_full_integrity_check() -> dict[str, Any]:
    ...
    try:
        ...
        results["overall_passed"] = (
            results["overall_passed"] and interface_result.passed
        )
    except Exception as e:
    ...
    try:
        ...
        results["overall_passed"] = (
            results["overall_passed"] and integration_result.passed
        )
    except Exception as e:
    ...
    try:
        ...
        for _prohibition, result in prohibition_results.items():
...
def get_system_integrity_status() -> str:
    ...
    try:
        ...
        if not results["overall_passed"]:
            ...
            # Check prohibitions
            prohibitions = (
                results.get("subsystems", {}).get("prohibitions", {}).get("results", {})
            )
            ...
            monitoring = (
                results.get("subsystems", {}).get("monitoring", {}).get("alerts", {})
            )
            for _category, alerts in monitoring.items():
                for alert in alerts:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run CodeMarshal integrity checks.")
    parser.add_argument(
        "--quick",
        action="store_true",
    ...
    parser.add_argument(
        "--full", action="store_true", help="Run full integrity check and output JSON"
    )
    parser.add_argument(
        "--validate-only", action="store_true", help="Run only validation checks"
    )
    parser.add_argument(
        "--monitor-only", action="store_true", help="Run only monitoring checks"
    )
    parser.add_argument(
        "--prohibitions-only", action="store_true", help="Run only prohibition checks"
    )
    ...
    if args.quick:
        ...
        for check_name in ["drift", "performance", "errors"]:
            try:
                ...
                monitoring_results[check_name] = [{"error": str(e)}]
