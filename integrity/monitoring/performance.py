...
from storage.layout import get_investigation_path, get_snapshot_paths
...
class PerformanceEvent:
    ...
    def to_dict(self) -> PerformanceEventDict:
        ...
        return cast(PerformanceEventDict, {
...
class PerformanceMonitor:
    ...
    def get_events(self, 
                   ...
                   limit: int = 1000) -> List[PerformanceEvent]:
        """
    ...
    def save_to_disk(self, path: Optional[Path] = None) -> Path:
        ...
        if path is None:
            ...
            path = inv_path / "performance" / f"events_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
...
def get_performance_monitor(context: Optional[RuntimeContext] = None) -> PerformanceMonitor:
    """
...
def monitor_performance(context: Optional[RuntimeContext] = None) -> Dict[str, Any]:
    """
