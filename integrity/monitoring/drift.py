...
# Storage imports
from storage.layout import get_investigation_path, get_snapshot_paths, get_observation_store_path
from storage.atomic import atomic_write
from storage.corruption import detect_corruption

# Observations imports
from observations.record.snapshot import load_snapshot, SnapshotMetadata, ObservationRecord
from observations.record.anchors import Anchor, get_anchor_id
...
class DriftDetection:
    ...
    snapshot_version1: Optional[str] = None
    snapshot_version2: Optional[str] = None
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'DriftDetection':
        ...
        return cls(
            ...
            snapshot_version1=str(data['snapshot_version1']) if data.get('snapshot_version1') else None,
            snapshot_version2=str(data['snapshot_version2']) if data.get('snapshot_version2') else None,
...
class DriftMonitor:
    ...
    def _load_snapshot(self, version: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def _compute_observation_hash(self, observation: Dict[str, Any]) -> str:
        ...
        cache_key = f"{observation.get('id', '')}:{json.dumps(observation, sort_keys=True)}"
    ...
    def record_drift(self,
                     ...
                     context: Optional[Dict[str, Any]] = None) -> DriftDetection:
        """
    ...
    def compare_snapshots(self,
                          version1: str,
                          version2: str,
                          method: DriftDetectionMethod = DriftDetectionMethod.HASH_COMPARISON) -> List[DriftDetection]:
        """
        ...
        if method == DriftDetectionMethod.HASH_COMPARISON:
            ...
            detections.extend(self._compare_by_content(snap1, snap2, version1, version2))
        else:
    ...
    def _compare_by_hash(self,
                         ...
                         snap2: Dict[str, Any],
                         version1: str,
                         version2: str) -> List[DriftDetection]:
        """
        ...
        for obs_id in all_ids:
            if obs_id in obs1 and obs_id not in obs2:
                # Observation removed
                detections.append(self.record_drift(
                ...
                if hash1 != hash2:
                    ...
                    detections.append(self.record_drift(
                        ...
                        evidence={
                            ...
                            "hash1": hash1,
                            "hash2": hash2,
                            "content_diff": self._compute_content_diff(obs1[obs_id], obs2[obs_id])
                        }
                    ))
        ...
        if snap1.get('input_hash') and snap2.get('input_hash'):
            if snap1['input_hash'] == snap2['input_hash'] and detections:
                ...
                detections.append(self.record_drift(
                    ...
                    method=DriftDetectionMethod.HASH_COMPARISON,
                    certainty=1.0,
                    description=f"Same input produced different observations (constitutional violation)",
                    snapshot_version1=version1,
                    snapshot_version2=version2,
                    evidence={
    ...
    def _compute_content_diff(self, obs1: Dict[str, Any], obs2: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def check_reproducibility(self,
                              ...
                              input_hash: Optional[str] = None) -> List[DriftDetection]:
        """
            ...
            for path in snapshot_paths:
                try:
                    ...
                    if snapshot.get('input_hash') == input_hash:
                        ...
                        historical_obs = {o.get('id'): o for o in snapshot.get('observations', [])}
                        ...
                        if historical_obs != current_obs:
                            detections.append(self.record_drift(
                                ...
                                method=DriftDetectionMethod.HASH_COMPARISON,
                                certainty=0.95,
                                description=f"Reproducibility failure: same input produced different observations",
    ...
    def detect_anchor_shift(self) -> List[DriftDetection]:
        ...
        try:
            from observations.record.anchors import load_anchors, Anchor
            ...
            if anchors_path.exists():
                ...
                for anchor in current_anchors:
                    ...
                    obs_store_path = get_observation_store_path(self.context.investigation_id)
    ...
    def get_detections(self,
                       ...
                       limit: int = 1000) -> List[DriftDetection]:
        """
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        critical_drift = sum(1 for d in detections if d.severity == DriftSeverity.CRITICAL)
        ...
        warning_messages = []
        if critical_drift > 0:
            warning_messages.append("🚨 Critical drift detected - truth preservation may be compromised")
        if DriftType.REPRODUCIBILITY_FAILED.name in drift_types:
            warning_messages.append("⚠️ Reproducibility failures detected - same inputs produce different outputs")
        if DriftType.ANCHOR_SHIFT.name in drift_types:
            warning_messages.append("⚠️ Anchor shifts detected - stable reference points have changed")
    ...
    def save_to_disk(self, path: Optional[Path] = None) -> Path:
        ...
        if path is None:
            inv_path = get_investigation_path(self.context.investigation_id)
            path = inv_path / "drift" / f"detections_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
...
def get_drift_monitor(context: Optional[RuntimeContext] = None) -> DriftMonitor:
    """
...
def monitor_drift(context: Optional[RuntimeContext] = None) -> Dict[str, Any]:
    """
    ...
    try:
            ...
            if len(snapshot_paths) >= 2:
                ...
                if detections:
                    summary["recent_comparison"] = {
                        ...
                        "critical_detections": len([d for d in detections if d.severity == DriftSeverity.CRITICAL])
                    }
        except Exception as e:
        ...
        if summary["health_status"] == "CRITICAL":
            ...
            summary["constitutional_articles"] = ["Article 3", "Article 9", "Article 13"]
        else:
...
def check_reproducibility(context: RuntimeContext,
...
def verify_immutability(context: RuntimeContext) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for i in range(1, len(sorted_paths)):
            ...
            for detection in detections:
                if detection.drift_type in [DriftType.OBSERVATION_CHANGED, DriftType.REPRODUCIBILITY_FAILED]:
                    violations.append({
