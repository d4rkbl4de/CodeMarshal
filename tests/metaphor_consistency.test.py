...
from bridge.entry.tui import TUIState
from lens.views.overview import OverviewView, InvestigationStage
...
class MetaphorConsistencyValidator:
    ...
    def validate_interface_metaphor(self, interface_file: Path) -> None:
        ...
        try:
            ...
            if 'single_focus' not in content.lower() and 'magnifying' not in content.lower():
                self.add_warning(
                    file_path=str(interface_file),
                    line=0,
...
def test_cli_metaphor_consistency():
    ...
    assert validator.is_compliant(), f"CLI has metaphor violations: {len(validator.violations)}"
...
def test_tui_metaphor_consistency():
    ...
    assert validator.is_compliant(), f"TUI has metaphor violations: {len(validator.violations)}"
...
def test_views_metaphor_consistency():
    ...
    assert validator.is_compliant(), f"Views have metaphor violations: {len(validator.violations)}"
...
def test_inquiry_metaphor_consistency():
    ...
    assert validator.is_compliant(), f"Inquiry has metaphor violations: {len(validator.violations)}"
...
def test_observation_metaphor_consistency():
    ...
    assert validator.is_compliant(), f"Observations have metaphor violations: {len(validator.violations)}"
...
def test_storage_metaphor_consistency():
    ...
    assert validator.is_compliant(), f"Storage has metaphor violations: {len(validator.violations)}"
...
def test_cross_module_metaphor_consistency():
    ...
    for file_path in key_files:
        ...
        if path.exists():
            ...
            term_count = 0
            for category, terms in validator.investigation_terms.items():
                for term in terms:
    ...
    assert validator.is_compliant(), f"Cross-module metaphor violations: {len(validator.violations)}"
...
def test_metaphor_enforcement_in_code():
    ...
    assert validator.is_compliant(), f"Metaphor enforcement violations: {len(validator.violations)}"
