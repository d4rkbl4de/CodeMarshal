...
class CrashRecoveryTester:
    ...
    def simulate_crash(self, session: SessionContext) -> bool:
        ...
        try:
            ...
            original_session_data = self.storage.load_session(session.snapshot_id)
...
def test_partial_data_recovery():
    ...
    try:
        ...
        assert len(recovered_session.observations) >= 2, "Should recover remaining observations"
...
def test_session_continuity_after_recovery():
    ...
    try:
        ...
        recovered_session.add_note(NoteEntry(
            id="note-002",
...
def test_concurrent_session_safety():
    ...
    try:
        ...
        session1_file = tester.temp_dir / "sessions" / "concurrent-test-1" / "session.json"
        ...
        assert len(recovered_session2.observations) > 0, "Session2 data should be intact"
        
        # Test that session1 can be recovered (or gracefully fails)
        recovered_session1 = tester.storage.load_session("concurrent-test-1")
...
def test_session_backup_and_restore():
    ...
    try:
        ...
        assert len(restored_session.observations) == 1, "Should restore original observation count"
...
def test_performance_recovery_large_sessions():
    ...
    try:
        ...
        assert len(recovered_session.observations) == 1000, "Should recover all observations"
        assert len(recovered_session.notes) == 100, "Should recover all notes"
        
        print(f"✅ Performance recovery large sessions: PASSED ({save_time:.2f}s save, {recovery_time:.2f}s recovery)")
