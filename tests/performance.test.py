...
""")
    ...
    def test_file_observation_performance(self):
        ...
        file_sight.observe(self.temp_dir / "small_project")
        result.end_measurement()

        # Test medium project
        result.start_measurement()
        file_sight.observe(self.temp_dir / "medium_project")
        result.end_measurement()

        # Test large project
        result.start_measurement()
        file_sight.observe(self.temp_dir / "large_project")
    ...
    def test_import_observation_performance(self):
        ...
        # Test small project
        result.start_measurement()
        import_sight.observe(self.temp_dir / "small_project")
        result.end_measurement()

        # Test medium project
        result.start_measurement()
        import_sight.observe(self.temp_dir / "medium_project")
        result.add_memory_measurement(self._get_memory_usage())
        result.end_measurement()

        # Test large project
        result.start_measurement()
        import_sight.observe(self.temp_dir / "large_project")
        result.add_memory_measurement(self._get_memory_usage())
        result.end_measurement()
    ...
    def test_pattern_analysis_performance(self):
        ...
        result.start_measurement()
        analyzer.analyze_coupling(small_obs)
        result.end_measurement()

        # Test medium project pattern analysis
        result.start_measurement()
        analyzer.analyze_coupling(medium_obs)
        result.add_memory_measurement(self._get_memory_usage())
        result.end_measurement()

        # Test large project pattern analysis
        result.start_measurement()
        analyzer.analyze_coupling(large_obs)
        result.add_memory_measurement(self._get_memory_usage())
        result.end_measurement()
    ...
    def test_concurrent_investigations(self):
        ...
        def run_investigation(project_path):
            ...
            file_sight.observe(project_path)
            end_time = time.time()
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            futures.append(
                executor.submit(run_investigation, self.temp_dir / "small_project")
            )
            futures.append(
                executor.submit(run_investigation, self.temp_dir / "medium_project")
            )
            futures.append(
                executor.submit(run_investigation, self.temp_dir / "large_project")
            )
    ...
    def test_memory_scaling(self):
        ...
        for size in project_sizes:
            ...
            for i in range(size):
                (test_dir / f"file_{i}.py").write_text(
                    f"def function_{i}(): return {i}"
                )

            result.start_measurement()
            file_sight.observe(test_dir)
...
def test_file_observation_performance():
    ...
    # Assertions
    assert result.max_duration < 1.0, (
        f"File observation too slow: {result.max_duration}s"
    )
    assert result.max_memory < 50, (
        f"File observation uses too much memory: {result.max_memory}MB"
    )
...
def test_import_observation_performance():
    ...
    assert result.max_duration < 2.0, (
        f"Import observation too slow: {result.max_duration}s"
    )
    assert result.max_memory < 100, (
        f"Import observation uses too much memory: {result.max_memory}MB"
    )
...
def test_pattern_analysis_performance():
    ...
    assert result.max_duration < 0.5, (
        f"Pattern analysis too slow: {result.max_duration}s"
    )
    assert result.max_memory < 150, (
        f"Pattern analysis uses too much memory: {result.max_memory}MB"
    )
...
def test_storage_performance():
    ...
    assert result.max_memory < 200, (
        f"Storage uses too much memory: {result.max_memory}MB"
    )
...
def test_export_performance():
    ...
    assert result.max_memory < 300, (
        f"Export uses too much memory: {result.max_memory}MB"
    )
...
def test_memory_scaling():
    ...
    if len(memory_values) >= 3:
        ...
        assert memory_values[-1] < memory_values[0] * 3, (
            f"Memory scaling too aggressive: {memory_values[-1]} vs {memory_values[0]}"
        )
...
def test_concurrent_performance():
    ...
    assert result.max_duration < 5.0, (
        f"Concurrent operations too slow: {result.max_duration}s"
    )
    assert len(result.measurements) == 3, (
        f"Expected 3 concurrent operations, got {len(result.measurements)}"
    )
