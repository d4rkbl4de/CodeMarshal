...
def prepare_for_large_run(target_path: str, expected_files: int = 50000):
    ...
    if observations_dir.exists():
        backup_manifest = backup_manager.create_full_backup(
            observations_dir, backup_id=f"pre_run_{int(time.time())}"
        )
    ...
    print("   Warning threshold: 2GB")
    print("   Critical threshold: 4GB")
    print("   Check interval: every 1000 files")

    # 6. Final verification
    print("\n✅ Preparation complete!")
    print("\nRun configuration:")
    print("   - Chunking: Enabled (1000 files per chunk)")
    print("   - Memory monitoring: Active")
    print("   - Backup strategy: Full + incremental")
    print("   - Transaction safety: Enabled")
    print("   - Corruption detection: Active")
    print()
