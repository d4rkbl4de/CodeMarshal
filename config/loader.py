...
from .defaults import SystemDefaults, OutputFormat, CommandType, BoundaryStrictness
from .user import (
    UserOverrides,
    from_cli_args,
    from_config_file,
...
class GlobalConfig:
    ...
    no_color: bool = False
    max_workers: int = 4
    cache_dir: Path = field(default_factory=lambda: Path.home() / ".codemarshal" / "cache")
    no_cache: bool = False
...
class Config:
    ...
    investigate: Optional[InvestigateConfig] = None
    ask: Optional[AskConfig] = None
    ...
    def __post_init__(self) -> None:
        ...
        command_to_config: Dict[CommandType, Optional[Any]] = {
    ...
    def _to_serializable_dict(self) -> Dict[str, Any]:
        ...
        if self.investigate:
            result["investigate"] = asdict(self.investigate, dict_factory=_strict_dict_factory)
        if self.ask:
            result["ask"] = asdict(self.ask, dict_factory=_strict_dict_factory)
        if self.patterns:
            result["patterns"] = asdict(self.patterns, dict_factory=_strict_dict_factory)
        if self.export:
...
class ConfigLoader:
    ...
    def __init__(self) -> None:
        ...
        self._user_overrides: Optional[UserOverrides] = None
        self._merged_config: Optional[Config] = None
    ...
    def load_from_cli(self, cli_args: Dict[str, Any]) -> Config:
        ...
        if cli_overrides.config_file:
            config_overrides = self._load_config_file_overrides(cli_overrides.config_file)
    ...
    def load_from_file(self, config_file: Path) -> Config:
        ...
        env_overrides = self._load_from_environment()
        self._user_overrides = self._merge_overrides(env_overrides, self._user_overrides)
    ...
    def _merge_overrides(self, base: UserOverrides, override: UserOverrides) -> UserOverrides:
    ...
    def _build_global_config(self) -> GlobalConfig:
            ...
            for user_field, config_field in field_mapping.items():
                ...
                if user_value is not None:
                    ...
                    if config_field in ('cache_dir', 'output_file'):
                        defaults_dict[config_field] = Path(user_value) if user_value else None
                    else:
        ...
        constructor_args: Dict[str, Any] = {
            k: v for k, v in defaults_dict.items() 
            if v is not None
        }
    ...
    def _build_command_configs(self, global_config: GlobalConfig) -> Dict[str, Any]:
        ...
        if self._user_overrides.witness:
            ...
            defaults_obj = SystemDefaults()
            witness_defaults: Dict[str, Any] = getattr(defaults_obj, 'WITNESS_DEFAULTS', {})
            
            path = witness_user.path or global_config.cache_dir / "witness"
            
            configs['witness'] = WitnessConfig(
                path=Path(path),
                output_format=witness_user.output_format or witness_defaults.get('output_format', OutputFormat.BOTH),
                ...
                max_file_size_mb=witness_user.max_file_size_mb or witness_defaults.get('max_file_size_mb', 10),
                ...
                exclude_patterns=witness_user.exclude_patterns or witness_defaults.get('exclude_patterns', []),
        ...
        if self._user_overrides.investigate:
            ...
            defaults_obj = SystemDefaults()
            investigate_defaults: Dict[str, Any] = getattr(defaults_obj, 'INVESTIGATE_DEFAULTS', {})
            ...
            configs['investigate'] = InvestigateConfig(
                ...
                boundary_strictness=investigate_user.boundary_strictness 
                    or investigate_defaults.get('boundary_strictness', BoundaryStrictness.STRICT),
            )
        ...
        if self._user_overrides.export:
            ...
            configs['export'] = ExportConfig(
                ...
                output_path=Path(export_user.output_path) if export_user.output_path else None,
...
def load_config(cli_args: Optional[Dict[str, Any]] = None) -> Config:
    """
