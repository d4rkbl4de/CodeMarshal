...
from observations.record.anchors import EvidenceAnchor
...
class BoundaryViolation:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "detected_at": self.evidence[0].timestamp.isoformat()
            if self.evidence
            else None,
        }
...
class BoundaryViolationDetector:
    ...
    def detect_all(self, snapshot: CodeSnapshot) -> tuple[BoundaryViolation, ...]:
        ...
        for rule in self._rules:
            rule_violations = self._detect_rule_violations(
                rule, boundary_observations, snapshot
            )
    ...
    def _is_rule_violation(
        self, rule: BoundaryRule, anchor: EvidenceAnchor, snapshot: CodeSnapshot
    ) -> bool:
        ...
        try:
            ...
            target_str = (
                str(target_path) if isinstance(target_path, Path) else target_path
            )
    ...
    def detect_import_violations(
        self, import_edges: list[tuple[str, str, EvidenceAnchor]]
    ...
    def _matches_import_rule(
        self, rule: BoundaryRule, source: str, target: str
    ) -> bool:
    ...
    def detect_path_violations(
        self, file_paths: list[tuple[str, EvidenceAnchor]]
    ) -> dict[str, list[EvidenceAnchor]]:
...
def calculate_violations(
    snapshot: CodeSnapshot, rules_config: dict | None = None
...
def test_boundary_violation_invariants() -> tuple[bool, list[str]]:
    ...
    empty_snapshot = CodeSnapshot(
        timestamp=datetime.now(UTC), root_path=Path("."), observations={}
    )
    ...
    violation = BoundaryViolation(rule_id="test", evidence=(), boundary_type="import")
