...
from . import complexity, coupling, density, uncertainty, violations
...
register_pattern(
    PatternMetadata(
        ...
        output_schema={
            ...
            "module_count": {
                "type": "int",
                "description": "Number of unique modules referenced",
            },
            "clustering_coefficient": {
                "type": "float",
                "description": "Import concentration (0-1)",
            },
            "distributions": {
                "type": "dict",
    ...
    PatternMetadata(
        ...
        output_schema={
            ...
            "adjacency": {
                ...
                "fields": {"node_labels": "list[str]", "matrix": "list[list[int]]"},
            },
            "components": {
                "type": "dict",
...
register_pattern(
    PatternMetadata(
        ...
        output_schema={
            "has_violations": {
                "type": "bool",
                "description": "Whether any violations were detected",
            },
            "violation_count": {
                "type": "int",
            ...
            "rule_ids": {
                "type": "list[str]",
                "description": "IDs of rules that were checked",
            },
...
register_pattern(
    PatternMetadata(
        ...
        output_schema={
            ...
            "confidence_score": {
                "type": "float",
                "description": "Overall confidence in observations (0-1)",
            },
            ...
            "missing_files": {
                "type": "list[str]",
                "description": "Files that were expected but not analyzed",
            },
            "missing_file_count": {
                "type": "int",
...
def get_pattern(pattern_type: PatternType) -> PatternMetadata | None:
...
def get_patterns_by_input(input_req: InputRequirement) -> list[PatternMetadata]:
    ...
    return [
        metadata
        for metadata in _PATTERN_REGISTRY.values()
...
def validate_pattern_inputs(
    pattern_type: PatternType, provided_inputs: dict[InputRequirement, Any]
) -> tuple[bool, list[str]]:
...
def execute_pattern(
    pattern_type: PatternType, inputs: dict[InputRequirement, Any]
) -> dict[str, Any]:
...
def validate_registry() -> tuple[bool, list[str]]:
    ...
    for pattern_type, metadata in _PATTERN_REGISTRY.items():
        ...
        if metadata.pattern_type != pattern_type:
            errors.append(
                f"Pattern type mismatch: {pattern_type} vs {metadata.pattern_type}"
            )
        ...
        for input_req in metadata.required_inputs:
            if not isinstance(input_req, InputRequirement):
                errors.append(
                    f"Invalid input requirement for {pattern_type}: {input_req}"
                )
