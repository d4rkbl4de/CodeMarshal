...
from typing import (
    ...
    DefaultDict, Counter, NamedTuple, FrozenSet, Pattern
)
...
class ModulePurpose:
    ...
    primary_signal: Optional[FunctionalitySignal] = None
    ...
    def describe(self) -> str:
        ...
        """
        if not self.signals:
            return "⚠️ No observable functionality signals found. Cannot determine purpose."
        ...
        for signal in self.signals:
            if signal.signal_type == FunctionalitySignal.ENTRY_POINT:
                signal_descriptions.append("• Serve as an entry point (if __name__ == '__main__')")
            elif signal.signal_type == FunctionalitySignal.CLI_BINDING:
                signal_descriptions.append("• Provide CLI functionality (argparse/click usage)")
            elif signal.signal_type == FunctionalitySignal.PUBLIC_API:
                signal_descriptions.append("• Expose a public API (__all__ or public names)")
            elif signal.signal_type == FunctionalitySignal.CLASS_CONSTRUCTOR:
                signal_descriptions.append("• Define class constructors (__init__ methods)")
                ...
                signal_descriptions.append("• Handle configuration (config parsing patterns)")
            elif signal.signal_type == FunctionalitySignal.DATA_TRANSFORMATION:
                signal_descriptions.append("• Transform data (to_/from_/parse/format patterns)")
            elif signal.signal_type == FunctionalitySignal.ERROR_HANDLING:
                signal_descriptions.append("• Handle errors (try/except/raise patterns)")
        
        # Add uncertainty notice
        if self.cannot_see:
            signal_descriptions.append(f"⚠️ Cannot see: {', '.join(self.cannot_see[:3])}")
        if self.uncertain_about:
            signal_descriptions.append(f"⁉️ Uncertain about: {', '.join(self.uncertain_about[:3])}")
...
class PurposeAnalysis:
    ...
    def get_modules_by_signal(self, signal_type: FunctionalitySignal) -> List[pathlib.Path]:
        """Get modules exhibiting a specific signal."""
        return [
            path for path, purpose in self.module_purposes.items()
    ...
    def describe_ecosystem(self) -> str:
        ...
        if entry_points > 0:
            parts.append(f"• {entry_points}/{total_modules} modules appear to be entry points")
        if cli_modules > 0:
            parts.append(f"• {cli_modules}/{total_modules} modules appear to provide CLI functionality")
        if api_modules > 0:
            parts.append(f"• {api_modules}/{total_modules} modules appear to expose public APIs")
        
        # Common exports (observable fact)
        if self.common_exports:
            top_exports = sorted(
                self.common_exports.items(),
                key=lambda x: x[1],
                reverse=True
            )[:5]
            parts.append("")
        ...
        parts.append("")
        parts.append("⚠️ Note: These are statistical observations, not architectural claims.")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'signal_frequency': {
                signal.name: count
                for signal, count in self.signal_frequency.items()
            },
            'limitations': [
                limitation.to_dict() if hasattr(limitation, 'to_dict')
                else str(limitation)
...
class PurposeAnalyzer:
    ...
    def __init__(self, import_observations: List[ImportObservation],
    ...
    def analyze_module(self, source_path: pathlib.Path, 
                       source_code: str) -> ModulePurpose:
        ...
        try:
            ...
            self.limitations.append(ValidationLimitation(
        ...
        return ModulePurpose(
            ...
            has_main_guard=any(s.signal_type == FunctionalitySignal.ENTRY_POINT 
                              for s in signals),
            has_argparse=any(s.signal_type == FunctionalitySignal.CLI_BINDING 
                            for s in signals),
            has_decorators=any(s.signal_type == FunctionalitySignal.DECORATOR_USAGE 
                              for s in signals),
            has_exports=any(s.signal_type == FunctionalitySignal.PUBLIC_API 
                           for s in signals)
        )
    
    def analyze_codebase(self, source_files: Dict[pathlib.Path, str]) -> PurposeAnalysis:
        """Analyze all source files for purpose signals."""
        module_purposes: Dict[pathlib.Path, ModulePurpose] = {}
        signal_counts: DefaultDict[FunctionalitySignal, int] = collections.defaultdict(int)
    ...
    def _analyze_import_relationships(self) -> List[Tuple[str, str, int]]:
        ...
        relationships: DefaultDict[Tuple[str, str], int] = collections.defaultdict(int)
...
def find_entry_points(analysis: PurposeAnalysis) -> List[Dict[str, Any]]:
    ...
    for module_path in entry_modules:
        ...
        signals = [s for s in purpose.signals 
...
def describe_module_purpose(analysis: PurposeAnalysis, 
                           module_path: pathlib.Path) -> Optional[str]:
...
def export_purpose_report(analysis: PurposeAnalysis, 
