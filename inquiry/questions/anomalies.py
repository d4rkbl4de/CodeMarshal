...
class ComparisonBaseline:
    ...
    distribution_type: str | None = None  # e.g., "normal", "skewed", "unknown"
...
class StatisticalAnomaly:
    ...
    def describe_deviation(self) -> str:
        ...
        if self.anomaly_type == AnomalyType.SIZE_OUTLIER:
            direction = (
                "larger" if self.metric_value > self.baseline.mean else "smaller"
            )
            return (
...
class AnomalyAnalysis:
    ...
    def summary(self) -> str:
        ...
        # Group by type
        type_counts: collections.defaultdict[str, int] = collections.defaultdict(int)
        ...
        if self.insufficient_data_for:
            parts.append("")
            parts.append(
                f"⚠️ Insufficient data for: {', '.join(self.insufficient_data_for[:3])}"
            )
    ...
    def get_most_deviant(self, count: int = 5) -> list[StatisticalAnomaly]:
        ...
        sorted_anomalies = sorted(
            self.anomalies, key=lambda a: abs(a.deviation_score), reverse=True
        )
...
class AnomalyDetector:
    ...
    def analyze_all(self) -> AnomalyAnalysis:
        ...
        confident_anomalies = [
            a for a in all_anomalies if a.confidence >= self.CONFIDENCE_THRESHOLD
        ]
        ...
        anomaly_rate = (
            len(confident_anomalies) / total_analyzed if total_analyzed > 0 else 0
        )
    ...
    def _detect_size_anomalies(
        self,
    ) -> tuple[list[StatisticalAnomaly], ComparisonBaseline | None]:
        ...
        for path, size in file_sizes:
            if size < lower_bound or size > upper_bound:
                ...
                z_score = (
                    (size - baseline.mean) / baseline.standard_deviation
    ...
    def _detect_complexity_anomalies(
        self,
    ) -> tuple[list[StatisticalAnomaly], ComparisonBaseline | None]:
            ...
            if baseline.standard_deviation > 0:
                ...
                if abs(z_score) > self.Z_SCORE_THRESHOLD:
                    ...
                    anomaly = StatisticalAnomaly(
                        ...
                        false_positive_risk=1.0 - confidence,
                        comparison_group=[
                            p for p, _ in complexity_metrics if p != path
                        ],
    ...
    def _detect_density_anomalies(
        ...
        for path, density in density_metrics.items():
            ...
            if percentile > 95 or percentile < 5:
                z_score = (
                    (density - baseline.mean) / baseline.standard_deviation
                    if baseline.standard_deviation > 0
                    else 0
                )
...
def export_anomaly_report(analysis: AnomalyAnalysis, output_path: str) -> None:
...
def filter_high_confidence_anomalies(
    analysis: AnomalyAnalysis, min_confidence: float = 0.7
) -> list[StatisticalAnomaly]:
    """Filter anomalies by confidence threshold."""
    return [a for a in analysis.anomalies if a.confidence >= min_confidence]


def get_anomaly_summary_by_path(analysis: AnomalyAnalysis, path: str) -> str | None:
