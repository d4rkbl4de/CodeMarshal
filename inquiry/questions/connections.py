...
from typing import (
    ...
    DefaultDict, Counter, NamedTuple, FrozenSet, DefaultDict
)
...
class ModuleNode:
    ...
    def out_degree(self) -> int:
        ...
        unique_targets = set(c.target_module for c in self.outgoing)
        return len(unique_targets)
    
    @property
    def in_degree(self) -> int:
        """Number of unique modules that import this module."""
        unique_sources = set(c.source_module for c in self.incoming)
...
class ConnectionGraph:
    ...
    def get_connections_between(self, source: str, target: str) -> List[ModuleConnection]:
    ...
    def get_transitive_dependencies(self, module_name: str, 
    ...
    def get_reverse_dependencies(self, module_name: str,
                                max_depth: int = 3) -> Set[str]:
    ...
    def to_adjacency_list(self) -> Dict[str, List[str]]:
        ...
        """
        adjacency: DefaultDict[str, List[str]] = collections.defaultdict(list)
...
class ConnectionAnalysis:
    ...
    coupling_patterns: Optional[CouplingPatterns] = None
    ...
    def describe_connectivity(self) -> str:
        ...
        parts.append(f"• {self.graph.unique_connections} unique module-to-module connections")
        
        # Degree distributions
        avg_out_degree = sum(
            deg * count for deg, count in self.graph.out_degree_distribution.items()
        ) / self.graph.module_count if self.graph.module_count > 0 else 0
        
        avg_in_degree = sum(
            deg * count for deg, count in self.graph.in_degree_distribution.items()
        ) / self.graph.module_count if self.graph.module_count > 0 else 0
        ...
        if self.hub_modules:
            parts.append(f"• {len(self.hub_modules)} modules with high connection counts")
        
        if self.strongly_connected_components:
            scc_sizes = [len(scc) for scc in self.strongly_connected_components if len(scc) > 1]
            if scc_sizes:
...
class ConnectionAnalyzer:
    ...
    def _extract_imports(self) -> None:
        ...
        if hasattr(self.snapshot, 'get_import_observations'):
            ...
            snapshot_dict = self.snapshot.to_dict() if hasattr(self.snapshot, 'to_dict') else {}
    ...
    def analyze(self) -> ConnectionAnalysis:
        ...
        connection_counts: DefaultDict[Tuple[str, str, ConnectionType], int] = (
    ...
    def _calculate_degree_distribution(self, nodes: Dict[str, ModuleNode], 
                                      direction: str) -> Dict[int, int]:
        """Calculate degree distribution (out or in)."""
        distribution: DefaultDict[int, int] = collections.defaultdict(int)
    ...
    def _find_strongly_connected_components(self, graph: ConnectionGraph) -> List[Set[str]]:
    ...
    def _find_hub_modules(self, graph: ConnectionGraph, 
...
def find_module_dependencies(analysis: ConnectionAnalysis, 
...
def find_connection_paths(analysis: ConnectionAnalysis,
                         source: str, 
                         target: str,
        ...
        if node:
            for connection in node.outgoing:
                if connection.target_module not in path:  # Avoid cycles
                    queue.append((connection.target_module, path + [connection.target_module]))
    
    return paths


def export_connection_graph(analysis: ConnectionAnalysis,
