...
class Thought(Generic[T]):
    ...
    confidence: float | None = field(default=None)  # Human's confidence, not system's
    ...
    def update(
        self, new_content: str, new_tags: set[str] | None = None
    ) -> "Thought[T]":
...
class ThoughtCollection:
    ...
    def load(cls, filepath: str) -> "ThoughtCollection":
        ...
        collection = cls(title=data["title"], description=data.get("description"))
