...
class HistoryStep:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "HistoryStep":
        ...
        return cls(
            ...
            metadata=tuple((item["key"], item["value"]) for item in data["metadata"]),
        )
        ...
        hash_input = json.dumps(
            {
                ...
                "metadata": sorted([f"{key}:{value}" for key, value in self.metadata]),
            },
            sort_keys=True,
        )
...
class InvestigationHistory:
    ...
    def get_step(self, step_id: UUID) -> HistoryStep | None:
        """
    ...
    def get_last_step(self) -> HistoryStep | None:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "InvestigationHistory":
        """Deserialize from dictionary."""
        steps = [HistoryStep.from_dict(step_data) for step_data in data["steps"]]
    ...
    def compute_integrity_hashes(self) -> dict[str, str]:
        ...
        """
        return {str(step.step_id): step.compute_integrity_hash() for step in self.steps}
...
class HistoryStorage:
    ...
    def load_history(
        self,
    ) -> tuple[InvestigationHistory | None, CorruptionState | None]:
        """
