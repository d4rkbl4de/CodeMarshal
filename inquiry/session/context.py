...
class SessionContext:
    ...
    def validate_existence(self, storage_path: str) -> tuple[bool, str | None]:
        """
        ...
        try:
            ...
            snapshot_version = get_snapshot_version(
                storage_path=storage_path, snapshot_id=self.snapshot_id
            )
            ...
            if anchor is None:
                return (
                    False,
                    f"Anchor {self.anchor_id} not found in snapshot {self.snapshot_id}",
                )
            ...
            if not is_valid_ref:
                return (
                    False,
                    f"Anchor {self.anchor_id} has invalid reference to snapshot",
                )
    ...
    def with_snapshot(
        self, new_snapshot_id: UUID, new_anchor_id: str | None = None
    ) -> "SessionContext":
        """
...
def create_context(
    ...
    context = SessionContext(
        snapshot_id=snapshot_id, anchor_id=anchor_id, question_type=question_type
    )
...
def create_initial_context(
    storage_path: str, initial_anchor_id: str = "root"
) -> SessionContext:
    """
...
def validate_context_references(context: SessionContext, storage_path: str) -> bool:
    """
