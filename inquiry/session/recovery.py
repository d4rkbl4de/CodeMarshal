...
from .history import InvestigationHistory, HistoryStep
...
class RecoveryState:
    ...
    last_valid_context: Optional[SessionContext]
    last_valid_history_step: Optional[HistoryStep]
    
    # Diagnostic metadata
    integrity_status: RecoveryIntegrity
    corruption_state: Optional[CorruptionState]
...
def inspect_session(
    ...
    can_resume_cleanly = _can_resume_cleanly(
        integrity_status,
        last_valid_context,
        last_valid_history_step
    )
    
    # Declare missing components
    missing_components = _identify_missing_components(
        context_result,
        history_result
    )
...
) -> Tuple[SessionContext, Optional[InvestigationHistory]]:
    """
...
def _check_corruption(
    context_result: Optional[AtomicReadResult],
    history_result: Optional[AtomicReadResult]
) -> Optional[CorruptionState]:
    """
    ...
    if history_result and history_result.data_bytes:
        corruption = detect_corruption(history_result.data_bytes, "investigation_history")
        if corruption:
            corruption_states.append(corruption)
...
def _determine_integrity(
    ...
    if (context_result and context_result.is_valid and
