...
class Exporter:
    ...
    @staticmethod
    def export_markdown(
        notes: Union[NoteEntry, List[NoteEntry]],
        tags: Optional[Dict[UUID, List[str]]] = None,
        ...
        for i, note in enumerate(notes_list, 1):
            ...
            if timeline:
                ...
                if history:
                    ...
                    for hist_time, action, old_content in history:
        ...
        for i, note in enumerate(notes_list, 1):
            if include_metadata:
                ...
                if timeline:
                    history = timeline.get_note_history(note.id)
                    if history:
                        output_lines.append(f"Edits: {len([h for h in history if h[1] == 'updated'])}")
    ...
    def export_to_file(
        ...
        **kwargs
        ...
        try:
            ...
            raise OSError(f"Failed to write export to {output_path}: {e}")
        
        return output_path


def _validate_and_normalize_notes(notes: Union[NoteEntry, List[NoteEntry]]) -> List[NoteEntry]:
    ...
    if isinstance(notes, NoteEntry):
        ...
        raise TypeError(f"Expected NoteEntry or list of NoteEntry, got {type(notes).__name__}")
