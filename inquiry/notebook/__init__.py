...
def export_notebook(
    ...
    **kwargs
    ...
    # Build tag mapping
    tags = {
        note.id: tag_manager.get_tags_for_note(note)
        for note in notes
    }
...
def validate_note_anchors(
    notebook: dict,
    note: Union[NoteEntry, UUID, str]
) -> List[str]:
    """
    ...
    if isinstance(note, NoteEntry):
        ...
        try:
            ...
            raise ValueError(f"Invalid UUID string: {note}")
    else:
        raise TypeError(f"Expected NoteEntry, UUID, or string, got {type(note).__name__}")
...
try:
    ...
    raise ImportError(f"Notebook module initialization failed: {e}")