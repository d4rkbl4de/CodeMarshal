...
class Timeline:
    ...
    def __init__(self) -> None:
        ...
        self._edit_history: list[
            tuple[datetime, str, UUID, str | None]
        ] = []  # (timestamp, action, note_id, old_content)
    ...
    def record_edit(self, note_id: UUID, old_content: str, edit_time: datetime) -> None:
        ...
        self._edit_history.append((edit_time, "updated", note_id, old_content))
    ...
    def replay_session(
        self, session_id: str
    ) -> list[tuple[datetime, str, NoteEntry, str | None]]:
        """
        ...
        session_notes = {
            n.id: n for n in self._notes_by_time if n.session_id == session_id
        }

        # Get all events for these notes, sorted chronologically
        session_events: list[tuple[datetime, str, NoteEntry, str | None]] = []
        for timestamp, action, note_id, old_content in self._edit_history:
            if note_id in session_notes:
                session_events.append(
                    (timestamp, action, session_notes[note_id], old_content)
                )

        return sorted(session_events, key=lambda x: x[0])  # Sort by timestamp

    def get_note_history(self, note_id: UUID) -> list[tuple[datetime, str, str | None]]:
        """
        ...
        """
        note_history: list[tuple[datetime, str, str | None]] = []
    ...
    def get_recent_notes(self, count: int = 10) -> list[NoteEntry]:
        ...
        """
        return sorted(self._notes_by_time, key=lambda n: n.created_at, reverse=True)[
            :count
        ]
