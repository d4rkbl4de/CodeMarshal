...
class NoteEntry:
    ...
    def __init__(
        ...
        for anchor in anchors:
            if not isinstance(anchor, ObservationAnchor):
                raise TypeError(
                    f"Expected ObservationAnchor, got {type(anchor).__name__}"
                )
    ...
    def update_content(
        self,
        new_content: str,
        new_anchors: list[ObservationAnchor] | None = None,
        ...
        self._updated_at = update_time if update_time is not None else datetime.now(UTC)
...
class NotebookManager:
    ...
    def create_note(
        ...
        """
        note = NoteEntry(
            content=content, anchors=anchors, author_id=author_id, session_id=session_id
        )
    ...
    def get_note(self, note_id: UUID) -> NoteEntry | None:
        """
        Get a note by ID.
