...
class TagManager:
    ...
    def add_tag(self, note: NoteEntry, tag: str) -> None:
        ...
        if not re.match(r"^[a-zA-Z0-9_-]+$", tag):
            raise ValueError(
                f"Tag '{tag}' contains invalid characters. Use letters, numbers, hyphens, underscores"
            )
    ...
    def search_by_keyword(
        self, keyword: str, notes: list[NoteEntry] | None = None
    ) -> list[NoteEntry]:
        """
    ...
    def search_by_anchor(
        self, anchor_id: str, notes: list[NoteEntry] | None = None
    ) -> list[NoteEntry]:
        """
