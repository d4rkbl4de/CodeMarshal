...
class RuntimeContext:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> RuntimeContext:
        ...
        # Convert limits
        for limit in ["max_file_size_bytes", "max_total_observation_size_bytes", "max_recursion_depth"]:
            if limit in data:
                converted_data[limit] = int(data[limit])
        
        # Convert enforcement_levels from list of strings to tuple of enums
        if "enforcement_levels" in data and isinstance(data["enforcement_levels"], list):
            levels: Tuple[EnforcementLevel, ...] = tuple(
                EnforcementLevel[level] if isinstance(level, str) else EnforcementLevel(level)
