...
class Runtime:
    ...
    def __init__(self, config: RuntimeConfiguration) -> None:
        ...
        try:
            ...
            self._logger.info(f"Runtime initialized successfully (mode={self._config.execution_mode.name})")
            
        except Exception as e:
            # Convert any exception during initialization to RuntimeInitializationError
            if not isinstance(e, (RuntimeInitializationError, ConstitutionValidationError)):
    ...
    def _validate_constitution(self) -> None:
        ...
        try:
            ...
            with open(self._config.constitution_path, 'r', encoding='utf-8') as f:
    ...
    def _create_runtime_context(self) -> RuntimeContext:
        ...
        try:
            ...
            constitution_hash = self._calculate_file_hash(self._config.constitution_path)
            
            # Calculate code version hash (hash of code root)
            code_version_hash = self._calculate_code_version_hash(self._config.code_root)
            ...
            raise RuntimeInitializationError(
                f"Failed to create runtime context: {e}",
                "context_creation"
            )
    ...
    def _calculate_file_hash(self, filepath: Path) -> str:
        ...
        try:
            ...
            raise RuntimeInitializationError(
                f"Failed to calculate file hash for {filepath}: {e}",
                "hash_calculation"
            )
    ...
    def _verify_integrity(self) -> None:
        ...
        try:
            ...
            if self._context.network_enabled != self._config.network_enabled:
                raise ConstitutionValidationError(
                    "Context network_enabled mismatch with configuration",
                    "TIER_1"
                )
            
            if self._context.mutation_allowed != self._config.allow_mutation:
                raise ConstitutionValidationError(
                    "Context mutation_allowed mismatch with configuration",
                    "TIER_1"
                )
    ...
    def execute(self) -> None:
        ...
        try:
            ...
            if self._state.current_phase.is_terminal():
                exit_code = 0 if self._state.current_phase == InvestigationPhase.TERMINATED_NORMAL else 1
                self._logger.info(f"Investigation completed with terminal state: {self._state.current_phase.name}")
            else:
                # This should never happen - engine should always reach terminal state
                self._logger.error(f"Investigation ended in non-terminal state: {self._state.current_phase.name}")
    ...
    def execute_observation_only(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self._logger.info(f"Observation phase completed: {len(observations)} observations")
    ...
    def start_investigation(
        ...
        """Start a new investigation session."""
        self._logger.info(f"Starting investigation: {target_path} (scope: {scope})")
    ...
    def resume_investigation(
        ...
        return {
            "session_id": session_id,
            "status": "resumed",
            "resumed_at": datetime.now(datetime.timezone.utc).isoformat(),
        }
    ...
    def fork_investigation(
        ...
        self._logger.info(f"Forking investigation: {source_session_id} -> {target_path}")
        ...
        return {
            ...
            "target_path": target_path,
            "forked_at": datetime.now(datetime.timezone.utc).isoformat(),
        }
...
def create_runtime(
    ...
    **kwargs
    ...
    if constitution_path is None:
        ...
        if constitution_path is None:
            raise RuntimeInitializationError(
                "Constitution file not found and no path provided",
                "path_resolution"
            )
