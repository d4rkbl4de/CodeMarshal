...
class Engine:
    ...
    def __init__(
        ...
        storage: InvestigationStorageInterface | None = None,
        memory_monitor: MemoryMonitorInterface | None = None,
    ):
        """
        ...
        self._observation_interface: ObservationInterface | None = None
        self._inquiry_interface: InquiryInterface | None = None
        ...
        if storage is not None:
            ...
            self._storage = module.InvestigationStorage()
    ...
    def register_observation_interface(self, interface: ObservationInterface) -> None:
        ...
        if self._observation_interface is not None:
            raise CoordinationError(
                "Observation interface already registered", "interface_registration"
            )
    ...
    def register_inquiry_interface(self, interface: InquiryInterface) -> None:
        ...
        if self._inquiry_interface is not None:
            raise CoordinationError(
                "Inquiry interface already registered", "interface_registration"
            )
    ...
    def register_lens_interface(self, interface: LensInterface) -> None:
        ...
        if self._lens_interface is not None:
            raise CoordinationError(
                "Lens interface already registered", "interface_registration"
            )
        ...
        try:
            ...
            if request.intent == HighLevelIntent.OBSERVE:
                if self._observation_interface is None:
                    raise CoordinationError(
                        "Observation interface not registered", "observation_only"
                    )
                ...
                observations = self._observation_interface.observe_directory(
                    request.target_path
                )
                ...
                if self._inquiry_interface is None:
                    raise CoordinationError(
                        "Inquiry interface not registered", "inquiry_only"
                    )
                ...
                if self._inquiry_interface is None:
                    raise CoordinationError(
                        "Inquiry interface not registered", "patterns_only"
                    )
                ...
                if self._lens_interface is None:
                    raise CoordinationError(
                        "Lens interface not registered", "presentation_only"
                    )
                ...
                raise CoordinationError(
                    f"Unsupported intent: {request.intent.name}", "intent_routing"
                )
    ...
    def execute_investigation(self) -> None:
        ...
        try:
            ...
            if observation_result.data:
                ...
                if observation_result.data.get("streaming"):
                    ...
                    obs_data = {
                        ...
                        "manifest": True,
                        "streaming": True,
                        "file_count": observation_result.data.get(
                            "observations_written", 0
                        ),
                        "boundary_crossings": observation_result.data.get(
                            "boundary_crossings", []
                        ),
                    ...
                    obs_id = self._storage.save_observation(
                        obs_data, self._context.session_id
                    )
            ...
            self._state.transition_to(
                InvestigationPhase.INQUIRY_ACTIVE, reason="Beginning inquiry phase"
            )
            ...
            if inquiry_result.data:
                ...
                content_str = str(inquiry_result.data)
                session_context = str(self._context.session_id)
                base_string = f"{content_str}:{session_context}"
                content_hash = hashlib.sha256(base_string.encode()).hexdigest()[:16]

                question_data = {
                    "id": f"q_{content_hash}",
                    "data": inquiry_result.data,
                    "phase": "inquiry_complete",
                    "timestamp": datetime.datetime.now().isoformat(),
                }
                q_id = self._storage.save_question(
                    question_data, self._context.session_id
                )
                question_ids.append(q_id)

            self._state.transition_to(
                InvestigationPhase.PATTERNS_CALCULATED, reason="Inquiry phase completed"
            )
            ...
            if patterns_result.data:
                ...
                content_str = str(patterns_result.data)
                session_context = str(self._context.session_id)
                base_string = f"{content_str}:{session_context}"
                content_hash = hashlib.sha256(base_string.encode()).hexdigest()[:16]

                pattern_data = {
                    "id": f"p_{content_hash}",
                    "data": patterns_result.data,
                    "phase": "patterns_complete",
                    "timestamp": datetime.datetime.now().isoformat(),
                }
                p_id = self._storage.save_pattern(
                    pattern_data, self._context.session_id
                )
    ...
    def start_investigation(
        ...
        if not target_path.exists():
            raise CoordinationError(
                f"Target path does not exist: {target_path}", "path_validation"
            )

        if not target_path.is_dir():
            raise CoordinationError(
                f"Target path must be a directory: {target_path}", "path_validation"
            )
        ...
        try:
            ...
            if result.success:
                # Store coordination result in history
                ...
                return {
                    ...
                    "observation_id": result.data.get("observation_id", "unknown"),
                    "execution_time_ms": result.execution_time_ms,
                }
            else:
                raise CoordinationError(
                    f"Failed to start investigation: {result.error_message}",
                    "investigation_start",
                )

        except Exception as e:
            self._logger.error(f"Investigation start failed: {e}")
            raise CoordinationError(
                f"Investigation start failed: {e}", "investigation_start"
            ) from e
    ...
    def submit_question(
        self, question_name: str, parameters: dict[str, Any], session_id: str
    ) -> str:
        """Submit question request for bridge compatibility."""
        # Create coordination request
        request = CoordinationRequest.create(
            intent=HighLevelIntent.QUERY,
            target_path=self._context.investigation_root,
            parameters={"question_name": question_name, **parameters},
        )

        result = self.coordinate(request)
        return (
            result.data.get("question_id", "unknown")
            if hasattr(result, "data") and result.data
            else "unknown"
        )

    def submit_pattern(
        self, pattern_name: str, parameters: dict[str, Any], session_id: str
    ) -> str:
        """Submit pattern detection request for bridge compatibility."""
        # Create coordination request
        request = CoordinationRequest.create(
            intent=HighLevelIntent.DETECT_PATTERNS,
            target_path=self._context.investigation_root,
            parameters={"pattern_name": pattern_name, **parameters},
        )

        result = self.coordinate(request)
        return (
            result.data.get("pattern_id", "unknown")
            if hasattr(result, "data") and result.data
            else "unknown"
        )
    ...
    def _is_phase_transition_allowed(
        self, current_phase: InvestigationPhase, target_phase: InvestigationPhase
    ) -> bool:
        """
        Check if a phase transition is allowed according to the investigation workflow.
        ...
        allowed_transitions = {
            # From BOOTSTRAPPED
            InvestigationPhase.BOOTSTRAPPED: [
                InvestigationPhase.ENFORCEMENT_ACTIVE,
            ],
            # From ENFORCEMENT_ACTIVE (observation phase)
            InvestigationPhase.ENFORCEMENT_ACTIVE: [
                InvestigationPhase.OBSERVATION_COMPLETE,
                InvestigationPhase.TERMINATED_ERROR,
            ],
            # From OBSERVATION_COMPLETE
            InvestigationPhase.OBSERVATION_COMPLETE: [
            ...
            # From PATTERNS_CALCULATED
            InvestigationPhase.PATTERNS_CALCULATED: [
                InvestigationPhase.PRESENTATION_ACTIVE,
                InvestigationPhase.TERMINATED_ERROR,
            ],
            # From PRESENTATION_ACTIVE
            InvestigationPhase.PRESENTATION_ACTIVE: [
                InvestigationPhase.TERMINATED_NORMAL,
                InvestigationPhase.TERMINATED_ERROR,
            ],
            ...
            InvestigationPhase.RECOVERY_NEEDED: [
                InvestigationPhase.RESUME_IN_PROGRESS,
            ],
            InvestigationPhase.RESUME_IN_PROGRESS: [
                InvestigationPhase.ENFORCEMENT_ACTIVE,
                InvestigationPhase.TERMINATED_ERROR,
