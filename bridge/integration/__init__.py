...
def check_integration_compliance(integration_type: IntegrationType) -> Dict[str, Any]:
    ...
    try:
        ...
        has_no_new_actions = all(
            "cannot" in cap.lower() or "no" in cap.lower()
            for cap in non_capabilities
        )
        
        # Check that integration doesn't bypass command authorization
        allowed_imports = cast(List[str], info["allowed_imports"])
        respects_commands = any(
            "bridge.commands" in imp for imp in allowed_imports
        )
        ...
        return {
            ...
            "compliance_summary": "PASS" if all([
                has_no_new_actions,
                respects_commands,
                not enhances_truth
            ]) else "FAIL"
