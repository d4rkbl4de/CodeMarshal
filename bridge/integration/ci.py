...
class CICheck:
    ...
    def to_ci_format(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "warning": "Uncertain results should not block CI" if not self.is_certain() else None
        }
...
class CIRunResult:
    ...
    def exit_code(self) -> int:
        ...
        certain_failures = [
            ...
            if c.verdict == CIVerdict.FAILURE and c.is_certain()
        ]
        if certain_failures:
            return 1
    ...
    def _generate_disclaimers(self) -> List[str]:
        ...
        if self.system_limitations:
            disclaimers.append(f"System limitations: {len(self.system_limitations)} declared")
...
class CIFormatter:
    ...
    def format_github_actions(result: CIRunResult) -> str:
        ...
        for check in result.checks:
            ...
            # Build message
            certainty_note = "" if check.is_certain() else f" [Uncertainty: {(1-check.certainty)*100:.1f}%]"
    ...
    def format_junit_xml(result: CIRunResult) -> str:
        ...
        test_suite.set("failures", str(len([c for c in result.checks if c.verdict == CIVerdict.FAILURE])))
        test_suite.set("errors", str(len([c for c in result.checks if c.verdict == CIVerdict.ERROR])))
        test_suite.set("skipped", str(len([c for c in result.checks if c.verdict == CIVerdict.UNCERTAIN])))
