...
class JSONExporter(BaseExporter):
    ...
    def export(
        ...
        if anchors:
            export_data["anchors"] = [self._serialize_anchor(anchor) for anchor in anchors]
        
        # Add notebook entries if provided
        if notebook_entries:
            export_data["notebook"] = [self._serialize_notebook_entry(entry) for entry in notebook_entries]
    ...
    def export(
        ...
        if snapshot:
            ...
            if hasattr(snapshot, 'created_at'):
                lines.append(f"- **Created**: {snapshot.created_at.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        lines.append("---")
        lines.append("")
        lines.append("*Export generated by CodeMarshal Truth-Preserving Investigation System*")
...
class PlainTextExporter(BaseExporter):
    ...
    def export(
        ...
        if notebook_entries:
            ...
            for entry in notebook_entries:
                lines.append(f"  Note from {entry.created_at.strftime('%Y-%m-%d %H:%M')}")
                lines.append(f"  Anchor: {entry.anchor_id}")
                lines.append("")
...
def get_exporter(format_type: Union[ExportFormat, str]) -> BaseExporter:
    """
