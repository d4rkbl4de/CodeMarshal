...
def integrity_check(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        _integrity_logger.debug(f"Integrity check: entering {module_name}.{func_name}")
...
class BridgeLayer:
    ...
    def _check_integration_compliance(self) -> dict[str, Any]:
        ...
        for integration_type in IntegrationType:
            ...
            if not report["constitutional"] or report["compliance_summary"] == "FAIL":
                violations.append(
                    f"{integration_type.value}: {report.get('error', 'Compliance failure')}"
                )
    ...
    def validate_command_flow(self, source: str, target: str, action: str) -> bool:
        """
    ...
    def get_constitutional_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            if integration_status["all_passed"]
            else "FAIL",
...
_bridge_instance: BridgeLayer | None = None
...
def validate_integration_use(
    integration_type: IntegrationType, proposed_action: str
) -> dict[str, Any]:
    """
    ...
    constitutional = all(
        article in info["constitutional_articles"] for article in ["12", "19"]
    )
