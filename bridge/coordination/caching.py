    ...
    def _make_space(self, required_bytes: int) -> bool:
        ...
        with self._lock:
            ...
            for key_hash, entry in self._cache.items():
                if entry.consistency_level in (
                    CacheConsistencyLevel.TRANSIENT,
            ...
            if bytes_freed >= required_bytes - available:
                logger.debug(
                    f"Evicted {entries_evicted} entries, freed {bytes_freed} bytes"
                )
        ...
        with entry_lock:
            with self._lock:
                ...
                if not self._make_space(estimated_size):
                    ...
                    self._notify_status(
                        "cache_full",
                        {"key": key.to_string(), "required_bytes": estimated_size},
                    )
                    return False
                ...
                logger.debug(
                    f"Cached entry: {key.to_string()} ({estimated_size} bytes)"
                )
                self._notify_status(
                    "entry_cached",
                    {
                ...
                if (
                    ...
                    and self._persistence_path
                    and consistency_level != CacheConsistencyLevel.STRONG
                ):
    ...
    def clear(self, include_strong: bool = False) -> dict[str, int]:
        ...
        with self._lock:
            ...
            for key_hash, entry in all_entries:
                if (
                    include_strong
                    or entry.consistency_level != CacheConsistencyLevel.STRONG
                ):
                    # Get lock for this entry
                    entry_lock = self._get_entry_lock(key_hash)
                    with entry_lock:
            ...
            result_stats.update(
                {
                    ...
                    "memory_usage_percent": (
                        self._current_bytes / self._max_bytes * 100
                    )
        ...
        try:
            for filepath in self._persistence_path.glob("cache_*.pkl"):
                try:
                    ...
                    logger.warning(
                        f"Failed to load persisted cache file {filepath}: {e}"
                    )
...
_system_cache: CacheManager | None = None
...
def reset_system_cache(include_strong: bool = False) -> dict[str, int]:
    ...
    with _cache_lock:
        if _system_cache:
            ...
            logger.warning(
                f"System cache reset: {sum(cleared.values())} entries cleared"
            )
            return cleared
