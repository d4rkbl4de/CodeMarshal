...
from .cli import main as cli_main, CodeMarshalCLI
...
def validate_entry_point(entry_point: str) -> bool:
    ...
    try:
        ...
        raise ImportError(f"Entry point '{entry_point}' is not accessible: {e}")
...
def __getattr__(name: str):
    ...
    if name in _PRIVATE_MODULES:
        raise AttributeError(
            f"Module '{name}' is private. {_PRIVATE_MODULES[name]}"
        )
