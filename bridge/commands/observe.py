...
class ObservationAuthorization:
    ...
    def is_lawful(
    ...
    ) -> tuple[bool, str | None, dict[str, list[str]] | None]:
        """
        ...
        if current_stage != WorkflowStage.ORIENTATION:
            ...
            return (
                False,
                f"Observation only allowed in ORIENTATION stage, not {stage_name}",
                None,
            )
        ...
        if session_context.has_observations and not request.parameters.get(
            "additional_view", False
        ):
            return (
                False,
                "Single-focus violation: must complete current observation first",
                limitations,
            )
    ...
    def _validate_target_path(
        cls, target_path: Path, session_context: SessionContext
    ) -> str | None:
        ...
        try:
            ...
            if target_path.is_symlink():
                return (
                    "Symlink observation requires explicit symlink_validation parameter"
                )
    ...
    @classmethod
    def _collect_limitations(cls, types: set[ObservationType]) -> dict[str, list[str]]:
    ...
    @classmethod
    def _check_type_conflicts(cls, types: set[ObservationType]) -> str | None:
        """Check for contradictory observation requests."""
        type_set = set(types)

        # Import and export sight might conflict on certain file types
        if (
            ObservationType.IMPORT_SIGHT in type_set
            and ObservationType.EXPORT_SIGHT in type_set
        ):
...
def _acknowledge_limitations(limitations: dict[str, list[str]] | None) -> bool:
    """
...
def _create_single_observation_request(
    obs_type: ObservationType, target_path: Path, session_id: str, **parameters: Any
) -> ObservationRequest:
...
def observe_file_structure(
    target_path: Path, session_id: str, include_hidden: bool = False
) -> ObservationRequest:
...
def observe_imports(
    target_path: Path, session_id: str, follow_imports: bool = False
) -> ObservationRequest:
