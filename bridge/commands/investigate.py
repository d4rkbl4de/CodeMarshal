...
class InvestigationRequest:
    ...
    target_path: Path
    session_id: str | None = None  # Required for RESUME/FORK, auto-generated for NEW
    ...
    def __post_init__(self) -> None:
        ...
        if self.type in [InvestigationType.RESUME, InvestigationType.FORK]:
            if not self.session_id:
                raise ValueError(
                    f"session_id required for {self.type.value} investigation"
                )
...
class InvestigationAuthorization:
    ...
    def is_lawful(
    ...
    ) -> tuple[bool, str | None, InvestigationState | None]:
        """
        ...
        if request.type == InvestigationType.NEW:
            ...
            return cls._validate_resume_investigation(
                request, runtime, existing_sessions
            )
    ...
    @classmethod
    def _validate_target_path(cls, target_path: Path) -> str | None:
...
def _get_constitutional_commitments(request: InvestigationRequest) -> dict[str, bool]:
    ...
    commitments = {
        ...
        "article_15_session_integrity": request.type
        != InvestigationType.NEW,  # Resume/fork only
...
def resume_investigation(
    session_id: str, target_path: str | Path | None = None, **parameters: Any
) -> InvestigationRequest:
...
def fork_investigation(
    source_session_id: str, target_path: str | Path, **parameters: Any
) -> InvestigationRequest:
