...
class ExportRequest:
    ...
    format: ExportFormat
    session_id: str
    scope: dict[str, Any] | None = None  # What subset to export
    ...
    def _validate_compatibility(self) -> str | None:
        """Validate that export type and format are compatible."""
        incompatible_pairs = {
            (
                ExportType.PATTERNS,
                ExportFormat.HTML,
            ): "Patterns cannot be exported as HTML",
        ...
        if self.type == ExportType.CONSTITUTIONAL and self.format not in [
            ExportFormat.JSON,
...
class ExportAuthorization:
    ...
    def is_lawful(
        ...
        disclosures = cls._generate_disclosures(
            request, session_context, incomplete_warning
        )
        ...
        if request.scope:
            scope_error = cls._validate_scope(
                request.scope, request.type, session_context
            )
            if scope_error:
    ...
    def _check_type_availability(
        cls, export_type: ExportType, session_context: SessionContext
    ) -> tuple[bool, str | None]:
    ...
    def _generate_disclosures(
        ...
        disclosures = {
            "truth_preservation_score": cls._TRUTH_PRESERVATION_SCORES.get(
                request.format, 0
            ),
            "format_constraints": cls._get_format_constraints(request.format),
            "limitations_included": True,  # Always include limitations
            "uncertainty_markers_included": request.type
            in [ExportType.PATTERNS, ExportType.CONSTITUTIONAL],
    ...
    def _validate_format_constraints(
        cls, request: ExportRequest, session_context: SessionContext
    ) -> str | None:
        ...
        if (
            ...
            and session_context.observations_incomplete
        ):
...
def _estimate_export_size(
    request: ExportRequest, session_context: SessionContext
) -> dict[str, Any]:
    """Provide honest estimate of export size and complexity."""
    base_sizes = {
        ExportType.OBSERVATIONS: session_context.observation_count
        * 1024,  # 1KB per observation
...
def export_observations_json(
    session_id: str, include_metadata: bool = True, **parameters: Any
) -> ExportRequest:
...
def export_notes_markdown(
    session_id: str, anchored_only: bool = True, **parameters: Any
) -> ExportRequest:
...
def export_constitutional_report(
    session_id: str, format: ExportFormat = ExportFormat.MARKDOWN, **parameters: Any
) -> ExportRequest:
