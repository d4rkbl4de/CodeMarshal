...
class QueryRequest:
    ...
    type: QueryType
    name: Union[QuestionName, PatternName]
    session_id: str
...
class QueryAuthorization:
    ...
    def is_lawful(
        ...
        if request.type not in allowed_types:
            ...
            return False, f"Query type '{request.type.value}' not allowed in {stage_name} stage"
        
        # 3. Stage-specific restrictions
        if current_stage == WorkflowStage.ORIENTATION:
            if (request.type == QueryType.QUESTION and 
                request.name not in cls._ORIENTATION_QUESTIONS):
    ...
    @classmethod
    def _validate_pattern_parameters(cls, params: Dict[str, Any]) -> Optional[str]:
