...
class LayoutContext:
    ...
    def __post_init__(self):
        ...
        if not self.layout_version.startswith("v"):
            raise ValueError(
                f"layout_version must start with 'v', got {self.layout_version}"
            )
        if not self.layout_version[1:].isdigit():
            raise ValueError(
                f"layout_version must be v<number>, got {self.layout_version}"
            )
...
def _validate_path_component(component: str, role: str = "path component") -> str:
    ...
    if "/" in component or "\\" in component:
        raise InvalidPathComponentError(
            f"{role} cannot contain path separators: {component}"
        )
    ...
    if any(ord(c) < 32 for c in component):
        raise InvalidPathComponentError(
            f"{role} contains control characters: {component}"
        )
...
def investigation_root(base_directory: Path, investigation_id: str) -> Path:
    """
...
def snapshot_directory(
    investigation_root: Path, snapshot_id: str, timestamp: str | None = None
) -> Path:
    """
...
def snapshot_file_path(
    snapshot_dir: Path, relative_file_path: str, preserve_extension: bool = True
) -> Path:
    """
    ...
    for part in parts:
        if part:  # Skip empty parts from leading/trailing slashes
            validated_parts.append(
                _validate_path_component(part, "file path component")
            )
...
def anchor_file_path(
    investigation_root: Path, anchor_type: str, anchor_id: str | None = None
) -> Path:
    """
...
def corruption_marker_path(
    investigation_root: Path, corrupted_path: Path, marker_suffix: str = ".corrupted"
) -> Path:
    """
...
def notebook_entry_file(
    investigation_root: Path, entry_id: str, timestamp: str | None = None
) -> Path:
    """
...
def pattern_file_path(
    investigation_root: Path, pattern_type: str, snapshot_id: str | None = None
) -> Path:
    """
...
def schema_file_path(
    investigation_root: Path, schema_name: str, version: str = "v1"
) -> Path:
    """
...
def version_marker_file(investigation_root: Path, version: str) -> Path:
    """
...
def get_storage_path(investigation_id: str, evidence_type: str) -> str:
    """
    Get a storage path for a specific evidence type.

    Args:
        investigation_id: Unique investigation identifier
        evidence_type: Type of evidence (e.g., 'observations', 'patterns', 'questions')

    Returns:
        Full path to the storage location for this evidence type
    """
    # Create layout context
    context = LayoutContext(investigation_id=investigation_id)

    # Map evidence types to storage roles
    evidence_type_map = {
        "observations": StorageRole.EVIDENCE,
        "snapshots": StorageRole.SNAPSHOTS,
        "anchors": StorageRole.ANCHORS,
        "metadata": StorageRole.METADATA,
        "notebook": StorageRole.NOTEBOOK,
        "patterns": StorageRole.PATTERNS,
        "corruption": StorageRole.CORRUPTION_MARKERS,
        "schemas": StorageRole.SCHEMAS,
        "versions": StorageRole.VERSIONS,
    }

    role = evidence_type_map.get(evidence_type.lower())
    if not role:
        raise ValueError(f"Unknown evidence type: {evidence_type}")

    # Get investigation root (assuming current directory for now)
    investigation_root = Path.cwd() / "observations" / investigation_id

    # Get all paths for this investigation
    all_paths = get_all_expected_paths(investigation_root, context)

    # Return the first path for the requested role
    paths = all_paths.get(role, [])
    if not paths:
        raise ValueError(f"No path found for evidence type: {evidence_type}")
