...
class ContentHash:
    ...
    @classmethod
    def compute(cls, content: bytes, algorithm: HashAlgorithm = HashAlgorithm.SHA256) -> ContentHash:
...
class FileObservation:
    ...
    size_bytes: int
    encoding: Optional[str] = None  # None means "could not determine"
    is_binary: bool = False
    is_symlink: bool = False
    symlink_target: Optional[str] = None  # Stored as string
...
class ImportObservation:
    ...
    def to_serializable(self) -> Dict[str, Any]:
        ...
        base.update({
            ...
            "imports": [imp.to_serializable() for imp in sorted(
                self.imports, 
...
class ExportDefinition:
    ...
    signature: Optional[str] = None  # For functions and methods
...
class ExportObservation:
    ...
    def to_serializable(self) -> Dict[str, Any]:
        ...
        base.update({
            ...
            "exports": [exp.to_serializable() for exp in sorted(
                self.exports,
                key=lambda x: (x.module_path, x.line_number, x.column_start)
            )],
            "module_count": self.module_count,
...
class BoundaryObservation:
    ...
    def to_serializable(self) -> Dict[str, Any]:
        ...
        base.update({
            ...
            "boundaries": [b.to_serializable() for b in sorted(
                self.boundaries,
                key=lambda x: x.path
            )],
            "boundary_violations": [
...
class FileEncoding:
    ...
    path: str  # Stored as string
    encoding: Optional[str]  # None means "could not determine"
...
class EncodingObservation:
    ...
    def to_serializable(self) -> Dict[str, Any]:
        ...
        base.update({
            ...
            "encodings": [enc.to_serializable() for enc in sorted(
                self.encodings,
                key=lambda x: x.path
            )],
...
def validate_evidence_structure(raw_data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    """Validate raw data against evidence structure requirements.
    
    Returns (is_valid, error_message)
    """
    # Check required top-level fields
    required_fields = {"evidence_id", "evidence_type", "schema_version", "content_hash", "context"}
    ...
    context_required = {"tool_version", "command_line", "working_directory", "created_at"}
...
def verify_evidence_integrity(evidence_data: Dict[str, Any]) -> Tuple[IntegrityStatus, Optional[str]]:
...
def deserialize_evidence(raw_bytes: bytes) -> Tuple[Optional[StoredEvidence], Optional[str]]:
