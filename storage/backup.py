...
from .corruption import verify_hash, CorruptionEvidence
...
class BackupManager:
    ...
    def create_full_backup(self, source_dir: Path, backup_id: Optional[str] = None) -> BackupManifest:
        """
        ...
        for rel_path, source_file in files_to_backup:
            ...
            try:
                ...
                if copied_count % 1000 == 0:
                    print(f"Backup progress: {copied_count}/{len(files_to_backup)} files", flush=True)
        ...
        print(f"✓ Full backup created: {backup_id} ({copied_count} files, {total_size//1024//1024}MB)", flush=True)
        return manifest
    ...
    def create_incremental_backup(self, 
        ...
        parent_manifest = self._manifests[parent_backup_id]
        ...
        for file_path in source_dir.rglob('*'):
            if file_path.is_file():
                ...
                if parent_file.exists():
                    ...
                    if (file_path.stat().st_mtime <= parent_file.stat().st_mtime and
                        file_path.stat().st_size == parent_file.stat().st_size):
                        should_backup = False
        ...
        print(f"✓ Incremental backup created: {backup_id} ({copied_count} new files, {total_size//1024//1024}MB)", flush=True)
        return manifest
    ...
    def cleanup_old_backups(self, keep_count: int = 5, days: int = 30) -> Dict[str, Any]:
        """
        ...
        for manifest in to_remove:
            ...
            try:
                ...
                print(f"⚠️ Failed to remove backup {manifest.backup_id}: {e}", flush=True)
