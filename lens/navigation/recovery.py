...
class RecoveryPath:
    ...
    target_view: ViewType | None
    """Specific view to use (if applicable)."""

    required_focus: str | None = None
...
class RecoveryRegistry:
    ...
    def _register_path(cls, path: RecoveryPath) -> None:
        ...
        if path.recovery_id in cls._registered_paths:
            raise ValueError(
                f"Recovery path with ID {path.recovery_id} already registered"
            )
    ...
    def _validate_recovery_constitutionality(cls, path: RecoveryPath) -> None:
        ...
        if path.is_safe_recovery and path.failure_type.requires_immediate_recovery:
            ...
            if (
                "emergency" not in path.justification.lower()
                and "degradation" not in path.justification.lower()
            ):
                raise ValueError(
    ...
    def get_recovery_path(
        ...
        matching_paths = [
            path
            for path in cls._registered_paths.values()
            if path.failure_type == failure_type
        ]

        if not matching_paths:
            raise ValueError(
                f"No recovery path registered for failure type: {failure_type}"
            )

        # If multiple paths, select based on severity/state
        if len(matching_paths) > 1:
            # Apply selection logic
            selected_path = cls._select_recovery_path(
                matching_paths, current_state, severity
            )
    ...
    def _select_recovery_path(
        ...
        if severity:
            severity_paths = [
                p
                for p in paths
                if ("emergency" in p.justification.lower() and severity == "high")
                or ("degradation" in p.justification.lower() and severity == "medium")
                or ("refresh" in p.justification.lower() and severity == "low")
            ]
            if severity_paths:
    ...
    def validate_recovery_possible(
        ...
        current_state: WorkflowState,
        session_context: SessionContext | None = None,
    ) -> tuple[bool, RecoveryPath | None, str | None]:
        """
...
class NavigationFailureDetector:
    ...
    def detect_failures(
        ...
        attempted_transition: tuple[WorkflowStage, ViewType | None] | None = None,
...
def validate_recovery_system_integrity() -> tuple[bool, str | None]:
    """
    ...
    try:
        ...
        for path in RecoveryRegistry.get_all_recovery_paths():
            ...
            if "Article" not in path.justification:
                return (
                    False,
                    f"Justification missing Article reference in path {path.recovery_id}",
                )
...
def get_recovery_system_summary() -> str:
    ...
    for failure_type in NavigationFailure:
        ...
        summary.append(
            f"    Immediate recovery: {failure_type.requires_immediate_recovery}"
        )
