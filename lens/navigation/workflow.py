...
from typing import Optional, Tuple, FrozenSet, ClassVar, Set
from uuid import UUID

# Allowed imports
from lens.philosophy import ProgressiveDisclosureRule, SingleFocusRule, ClarityRule, NavigationRule
...
class WorkflowTransition:
    ...
    def __init__(
        ...
        reason: str,
        required_view: Optional[ViewType] = None
    ) -> None:
        """
    ...
    def _validate_transition(self, from_stage: WorkflowStage, to_stage: WorkflowStage) -> None:
    ...
    def is_valid(self, current_view: Optional[ViewType] = None) -> Tuple[bool, Optional[str]]:
        """
...
class WorkflowState:
    ...
    def can_transition_to(self, target_stage: WorkflowStage) -> Tuple[bool, Optional[str]]:
        """
...
class WorkflowEngine:
    ...
    def validate_transition(
        ...
        target_view: Optional[ViewType] = None
    ) -> Tuple[bool, Optional[str], Optional[WorkflowTransition]]:
        """
        ...
        if current_state.current_stage == target_stage:
            if target_view and target_view != current_state.current_view:
                ...
                return True, None, WorkflowTransition(
    ...
    def get_available_transitions(
        ...
        if next_stage:
            ...
            if can_transition:
                available.add(WorkflowTransition(
                    from_stage=current_state.current_stage,
                    to_stage=next_stage,
    ...
    def get_initial_state(self, session_id: Optional[UUID] = None) -> WorkflowState:
        """
...
class Step:
    ...
    next_up: Optional[WorkflowStage] = None
    previous: Optional[WorkflowStage] = None
...
class WorkflowNavigator:
    ...
    def get_step(self, status: str) -> Optional[Step]:
        """
...
def validate_workflow_integrity() -> Tuple[bool, Optional[str]]:
    """
    ...
    try:
        ...
        for i in range(len(WorkflowEngine.CANONICAL_ORDER) - 1):
            ...
            if not can_transition:
                return False, f"Canonical transition {current}→{next_stage} failed: {reason}"
        ...
        for i in range(1, len(WorkflowEngine.CANONICAL_ORDER)):
            ...
            if can_transition:
                return False, f"Backward transition {current}→{previous} should not be allowed"
        ...
        for i in range(len(WorkflowEngine.CANONICAL_ORDER) - 2):
            ...
            if can_transition:
                return False, f"Stage skipping {current}→{skip_target} should not be allowed"
...
def get_canonical_workflow_description() -> str:
    ...
    description_lines.append("- Completion of THINKING stage indicates workflow progress, not investigation completion")
