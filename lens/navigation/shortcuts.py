...
from typing import Optional, Tuple, FrozenSet, List, Dict, Any, ClassVar
...
class Shortcut:
    ...
    allowed_views: Optional[FrozenSet[ViewType]] = None
    ...
    def __post_init__(self) -> None:
        ...
        if not self.shortcut_id:
            object.__setattr__(self, 'shortcut_id', 
    ...
    def _validate_justification(self) -> None:
        ...
        principles = [
            "Article 4", "Article 6", "Article 7", "Article 14",
            "Progressive Disclosure", "Linear Investigation",
            "Clear Affordances", "Graceful Degradation"
        ]
    ...
    def is_available(
        ...
        shortcut_history: Optional[List["Shortcut"]] = None
        ...
        if current_state.current_stage != self.from_stage:
            return False, f"Current stage {current_state.current_stage} does not match from_stage {self.from_stage}"
        
        # 2. Check view constraints
        if self.allowed_views is not None:
            if current_state.current_view not in self.allowed_views:
                return False, f"Current view {current_state.current_view} not in allowed views"
        ...
        if self.requires_same_focus:
            ...
            if session_context and session_context.anchor_id != current_state.focus_id:
                return False, "Focus mismatch between workflow state and session context"
        
        # 4. Check usage limits
        if shortcut_history and self.maximum_uses is not None:
            same_type_uses = sum(
                1 for s in shortcut_history 
                if s.shortcut_type == self.shortcut_type
            )
        ...
        if shortcut_history and self.cooldown_seconds is not None:
            ...
            recent_uses = [
                s for s in shortcut_history 
                if s.shortcut_type == self.shortcut_type
            ]
            if recent_uses:
                latest_use = max(s.created_at for s in recent_uses)
                time_since_use = (datetime.now(timezone.utc) - latest_use).total_seconds()
                if time_since_use < self.cooldown_seconds:
                    return False, f"Cooldown active ({int(self.cooldown_seconds - time_since_use)}s remaining)"
        ...
        if to_index > from_index:
            return False, f"Cannot use shortcut to skip forward from {self.from_stage} to {self.to_stage} (Article 4)"
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "allowed_views": [v.value for v in self.allowed_views] if self.allowed_views else None,
...
class ShortcutRegistry:
    ...
    def initialize(cls) -> None:
        ...
        # Backward jumps
        cls._register_shortcut(Shortcut(
        ...
        cls._register_shortcut(Shortcut(
            ...
            requires_same_focus=True,
            allowed_views=frozenset([ViewType.CONNECTIONS_GRAPH, ViewType.CONNECTIONS_TABLE]),
            maximum_uses=10,
            cooldown_seconds=2
        ))
    ...
    def _register_shortcut(cls, shortcut: Shortcut) -> None:
        ...
        if shortcut.shortcut_id in cls._registered_shortcuts:
            raise ValueError(f"Shortcut with ID {shortcut.shortcut_id} already registered")
    ...
    def _validate_shortcut_constitutionality(cls, shortcut: Shortcut) -> None:
        ...
        if shortcut.from_stage == shortcut.to_stage and shortcut.allows_focus_change:
            ...
            if "focus" not in shortcut.justification.lower() and "context" not in shortcut.justification.lower():
                raise ValueError(
    ...
    def get_available_shortcuts(
        ...
        for shortcut in cls._registered_shortcuts.values():
            ...
            if shortcut.shortcut_type in [ShortcutType.RETURN_TO_LAST_VALID, 
    ...
    @classmethod
    def get_shortcut_by_id(cls, shortcut_id: str) -> Optional[Shortcut]:
        """Get shortcut by ID."""
        if not cls._initialized:
            cls.initialize()
    ...
    def get_shortcuts_by_type(cls, shortcut_type: ShortcutType) -> List[Shortcut]:
        ...
        return [
            s for s in cls._registered_shortcuts.values()
    ...
    def validate_shortcut_use(
        ...
        session_context: Optional[SessionContext] = None,
        shortcut_history: Optional[List[Shortcut]] = None
    ) -> Tuple[bool, Optional[Shortcut], Optional[str]]:
        """
        ...
        available_shortcuts = [
            s for s in cls.get_available_shortcuts(current_state, session_context, shortcut_history)
            if s.shortcut_type == shortcut_type
        ]
        
        if not available_shortcuts:
            return False, None, f"No available shortcuts of type {shortcut_type} from current state"
...
def validate_shortcuts_integrity() -> Tuple[bool, Optional[str]]:
    """
    ...
    try:
        ...
        for shortcut_id, shortcut in ShortcutRegistry._registered_shortcuts.items():
            ...
            if list(ShortcutRegistry._registered_shortcuts.values()).count(shortcut) > 1:
