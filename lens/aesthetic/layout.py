...
class RegionRule:
    ...
    prohibited_with: Set[LayoutRegion] = field(default_factory=lambda: cast(Set[LayoutRegion], set()))
    required_with: Set[LayoutRegion] = field(default_factory=lambda: cast(Set[LayoutRegion], set()))
    max_concurrent: Optional[int] = None
    
    def __post_init__(self) -> None:
        """Validate region rule consistency."""
        if (LayoutConstraint.SINGLE_FOCUS in self.constraints and 
...
class LayoutSystem:
    ...
    def __init__(self) -> None:
        ...
        self._current_primary: Optional[LayoutRegion] = None
    ...
    def _build_rules(self) -> Dict[LayoutRegion, RegionRule]:
        ...
        return {
            ...
            LayoutRegion.THINKING: RegionRule(
                ...
                required_with={LayoutRegion.PRIMARY},  # Must have something to think about
                prohibited_with={
    ...
    def validate_regions(self, regions: Set[LayoutRegion]) -> List[str]:
        ...
        for region in regions:
            ...
            if (LayoutConstraint.MUST_INTERRUPT in rule.constraints and 
                self._has_persistent_regions(regions)):
                violations.append(
                    f"{region.name} must interrupt but persistent regions present"
                )
            
            if (LayoutConstraint.MUST_BE_ISOLATED in rule.constraints and 
                len(regions) > 1):
                violations.append(
        ...
        for position, count in position_counts.items():
            if count > 1:
                ...
                if LayoutRegion.PRIMARY in regions_at_position:
                    ...
                    regions_to_move = cast(
                        List[LayoutRegion], 
                        regions_at_position[1:]
                    )
    ...
    def get_constraint_explanations(self) -> Dict[LayoutConstraint, str]:
        ...
        return {
            LayoutConstraint.SINGLE_FOCUS: 
                "Only one active region at a time to prevent cognitive overload",
            LayoutConstraint.MUST_INTERRUPT: 
                "Must break normal flow to ensure visibility (e.g., warnings)",
            LayoutConstraint.MUST_BE_PROXIMATE: 
                "Must be near related content to preserve truth relationships",
            LayoutConstraint.MUST_BE_ISOLATED: 
                "Cannot appear with other regions to prevent competition",
            LayoutConstraint.PASSIVE_ONLY: 
                "Cannot accept user input to maintain focus on primary",
            LayoutConstraint.REQUIRES_ANCHOR: 
                "Must be anchored to specific content to prevent floating ideas",
            LayoutConstraint.CANNOT_COMPETE: 
                "Must not draw attention away from primary content",
        }
