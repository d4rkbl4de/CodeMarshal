...
from . import aesthetic, indicators, navigation, philosophy, views
...
AestheticConstraintValue = TextRole | SemanticColor | int | bool | str
...
class LensConfiguration:
    ...
    def create(
        cls,
        mode: InvestigationMode,
        aesthetic_constraints: dict[str, AestheticConstraintValue] | None = None,
        layout_regions: set[LayoutRegion] | None = None,
    ...
    def _validate(self) -> None:
        ...
        if not self.layout_regions:
            raise ValueError(
                "Lens configuration must specify at least one layout region"
            )
...
class LensSystem:
    ...
    def __init__(self) -> None:
        ...
        self._current_mode: InvestigationMode | None = None
    ...
    def validate_transition(
        self, from_mode: InvestigationMode | None, to_mode: InvestigationMode
    ) -> list[str]:
        """
        ...
        if from_mode == InvestigationMode.OBSERVING and to_mode in [
            InvestigationMode.PATTERNING,
            InvestigationMode.THINKING,
        ]:
            violations.append(
        ...
        if (
            from_mode == InvestigationMode.THINKING
            and to_mode == InvestigationMode.OBSERVING
        ):
            violations.append(
    ...
    def _apply_aesthetic_constraints(
        self, content: dict[str, Any], config: LensConfiguration
    ) -> dict[str, Any]:
