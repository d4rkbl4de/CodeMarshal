...
class ConnectionEndpoint:
    ...
    display_name: str
    location: str | None = None  # Path, line number, etc.
...
class DeclaredConnection:
    ...
    declared_by: str | None = None  # Who/what declared it
    ...
    def __post_init__(self) -> None:
        ...
        if not self.strength.is_allowed:
            raise ValueError(
                f"Connection strength {self.strength} not allowed in connections view"
            )
        ...
        for indicator in inference_indicators:
            if indicator in evidence_lower:
                raise ValueError(
                    f"Connection evidence contains inference: '{indicator}'"
                )
...
class ConnectionsView:
    ...
    def __init__(
        ...
        connections: tuple[DeclaredConnection, ...],
        config: ConnectionsRenderConfig | None = None,
    ) -> None:
        """
        ...
        self._config: ConnectionsRenderConfig = (
            config or ConnectionsRenderConfig.default()
        )

        # Apply philosophy rules
        self._apply_philosophy_rules()

    def _validate_connections_are_declared(
        self, connections: tuple[DeclaredConnection, ...]
    ) -> None:
        """
        ...
        for conn in connections:
            ...
            if not isinstance(conn, DeclaredConnection):
                raise TypeError(
                    f"Connection must be DeclaredConnection, got {type(conn)}"
                )

            # Check for inference
            if conn.strength == ConnectionStrength.INFERRED:
                raise ValueError(
                    f"Connection {conn.id} has INFERRED strength - not allowed"
                )
    ...
    def _organize_by_endpoint(self) -> dict[str, Any]:
        ...
        for endpoint_id in sorted(groups_by_from.keys()):
            ...
            sorted_conns = sorted(
                endpoint_conns, key=lambda c: c.to_endpoint.display_name
            )
    ...
    def _organize_by_type(self) -> dict[str, Any]:
        ...
        groups_by_type: dict[ConnectionType, list[DeclaredConnection]] = defaultdict(
            list
        )
        ...
        for conn_type in ConnectionType:
            if conn_type in groups_by_type:
                ...
                sorted_conns = sorted(
                    type_conns, key=lambda c: c.timestamp, reverse=True
                )
    ...
    def _organize_by_strength(self) -> dict[str, Any]:
        ...
        groups_by_strength: dict[ConnectionStrength, list[DeclaredConnection]] = (
            defaultdict(list)
        )
        ...
        for strength in ConnectionStrength:
            if strength in groups_by_strength:
                ...
                sorted_conns = sorted(
                    strength_conns, key=lambda c: c.connection_type.name
                )
    ...
    def _organize_chronologically(self) -> dict[str, Any]:
        ...
        sorted_conns = sorted(
            self._connections, key=lambda c: c.timestamp, reverse=True
        )
        ...
        return {
            ...
            "time_range": {
                "earliest": sorted_conns[-1].timestamp.isoformat()
                if sorted_conns
                else None,
                "latest": sorted_conns[0].timestamp.isoformat()
                if sorted_conns
                else None,
            },
        }
    ...
    def _prepare_connection_display(self, conn: DeclaredConnection) -> dict[str, Any]:
        ...
        if self._config.highlight_current_focus and self._context.current_focus:
            focus_id = self._context.current_focus
            if (
                focus_id in [conn.from_endpoint.id, conn.to_endpoint.id]
                or (
                    conn.from_endpoint.location
                    and focus_id in conn.from_endpoint.location
                )
                or (conn.to_endpoint.location and focus_id in conn.to_endpoint.location)
            ):
    ...
    def _prepare_group_display(self, group: ConnectionGroup) -> dict[str, Any]:
        ...
        display: dict[str, Any] = {
            ...
            "connections": [
                self._prepare_connection_display(conn) for conn in group.connections
            ],
    ...
    def _apply_display_rules(self, organized: dict[str, Any]) -> dict[str, Any]:
        ...
        result["display_stats"] = {
            ...
            if displayed_count < total_connections
            else None,
        }
        ...
        if "groups" in organized:
            result["display_groups"] = [
                self._prepare_group_display(group) for group in organized["groups"]
            ]
    ...
    def validate_integrity(self) -> list[str]:
        ...
        for conn in self._connections:
            ...
            for indicator in inference_indicators:
                if indicator in conn.evidence.lower():
                    violations.append(
                        f"Connection {conn.id} evidence contains inference: '{indicator}'"
                    )
        ...
        connections_with_limitations = [
            c for c in self._connections if c.has_limitations
        ]
        if connections_with_limitations and not self._config.show_limitations:
            violations.append(
                f"{len(connections_with_limitations)} connections have limitations but they are hidden"
            )
        ...
        for conn in self._connections:
            for word in causation_words:
                if word in conn.evidence.lower():
                    violations.append(
                        f"Connection {conn.id} implies causation: '{word}'"
                    )

        return violations

    def get_connections_involving(
        self, endpoint_id: str
    ) -> tuple[DeclaredConnection, ...]:
        """
        ...
        for conn in self._connections:
            if (
                conn.from_endpoint.id == endpoint_id
                or conn.to_endpoint.id == endpoint_id
            ):
                result.append(conn)
    ...
    def get_endpoint_summary(self, endpoint_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "connection_types": {
                conn_type.name: sum(
                    1 for c in connections if c.connection_type == conn_type
                )
                for conn_type in ConnectionType
            },
        }
    ...
    def create_test_view(cls) -> ConnectionsView:
        ...
        conn1 = DeclaredConnection(
            ...
            declared_limitations=(
                "Only shows explicit anchor",
                "Does not show if note understands observation",
            ),
        )
        ...
        return cls(TestContext(), (conn1, conn2, conn3, conn4, conn5))
...
def main() -> None:
    ...
    # Test empty state
    print("\n=== EMPTY STATE TEST ===")
    empty_view = ConnectionsView(view._context, ())
