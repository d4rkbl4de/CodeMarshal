...
class PatternArtifact:
    ...
    def __post_init__(self) -> None:
        ...
        if self.detected_at.tzinfo is None:
            object.__setattr__(
                self, "detected_at", self.detected_at.replace(tzinfo=UTC)
            )
...
class PatternsView:
    ...
    def __init__(
        ...
        config: PatternsRenderConfig | None = None,
    ) -> None:
        """
    ...
    def _validate_patterns_are_artifacts(
        self, patterns: tuple[PatternArtifact, ...]
    ) -> None:
        """
        ...
        for pattern in patterns:
            ...
            if hasattr(pattern, "computation_trace"):
                raise ValueError(
                    "Pattern artifact contains computation trace - this is a violation"
                )

            if hasattr(pattern, "intermediate_results"):
                raise ValueError(
                    "Pattern artifact contains intermediate results - this is a violation"
                )

            if hasattr(pattern, "inference_chain"):
                raise ValueError(
                    "Pattern artifact contains inference chain - this is a violation"
                )
    ...
    def render(
        self, mode: PatternsDisplayMode = PatternsDisplayMode.BY_CONFIDENCE
    ) -> dict[str, Any]:
        """
        ...
        rendered.update(
            self._get_view_metadata(len(filtered_patterns), len(self._patterns))
        )
    ...
    def _organize_patterns(
        self, patterns: tuple[PatternArtifact, ...], mode: PatternsDisplayMode
    ) -> dict[str, Any]:
        """
    ...
    def _organize_by_confidence(
        ...
        organized: dict[str, Any] = {"organization": "by_confidence", "groups": []}
                ...
                organized["groups"].append(
                    {
                        ...
                        "patterns": [
                            self._prepare_pattern_display(p) for p in group_patterns
                        ],
                    }
                )
    ...
    def _organize_by_type(
        ...
        organized: dict[str, Any] = {"organization": "by_type", "groups": []}
    ...
    def _organize_by_scope(
        ...
        organized: dict[str, Any] = {"organization": "by_scope", "groups": []}
    ...
    def _organize_chronologically(
        ...
        return {
            ...
            "time_range": {
                "start": sorted_patterns[-1].detected_at.isoformat()
                if sorted_patterns
                else None,
                "end": sorted_patterns[0].detected_at.isoformat()
                if sorted_patterns
                else None,
            },
        }

    def _organize_unfiltered(
        self, patterns: tuple[PatternArtifact, ...]
    ) -> dict[str, Any]:
    ...
    def _prepare_pattern_display(self, pattern: PatternArtifact) -> dict[str, Any]:
        ...
        if (
            self._config.highlight_boundaries
            and pattern.pattern_type == PatternType.BOUNDARY_VIOLATION
        ):
    ...
    def _apply_display_rules(self, organized: dict[str, Any]) -> dict[str, Any]:
        ...
        if displayed_patterns < total_patterns:
            ...
            result["suggestion"] = (
                "Apply fewer filters or increase display limit to see more patterns"
            )
        else:
        ...
        for pattern in self._patterns:
            if pattern.reference_count == 0:
                warnings.append(
                    f"Pattern '{pattern.name}' has no supporting references"
                )
        ...
        if low_confidence_count > len(self._patterns) / 2:
            warnings.append(
                f"Majority ({low_confidence_count}/{len(self._patterns)}) patterns have low confidence"
            )
    ...
    def _render_empty_state(self) -> dict[str, Any]:
        ...
        if len(self._patterns) == 0:
            ...
            if low_conf_count == len(self._patterns):
                reasons.append(
                    f"All {low_conf_count} patterns have low confidence and are hidden by filter"
                )
            ...
            if no_ref_count == len(self._patterns):
                reasons.append(
                    f"All {no_ref_count} patterns lack references and are hidden by filter"
                )
    ...
    def _get_view_metadata(
        self, displayed_count: int, total_count: int
    ) -> dict[str, Any]:
    ...
    def validate_integrity(self) -> list[str]:
        ...
        for pattern in self._patterns:
            if not pattern.uncertainty_reason.strip():
                violations.append(
                    f"Pattern '{pattern.name}' does not declare uncertainty (Article 11)"
                )

            if len(pattern.known_limitations) == 0:
                violations.append(
                    f"Pattern '{pattern.name}' has no declared limitations (Article 11)"
                )
    ...
    def create_test_view(cls) -> PatternsView:
        ...
        pattern2 = PatternArtifact(
            ...
            known_limitations=(
                "Does not measure cognitive complexity",
                "Ignores linear but long functions",
            ),
        ...
        pattern3 = PatternArtifact(
            ...
            references=(),  # No references - low confidence
            known_limitations=(
                "Density thresholds arbitrary",
                "Ignores import frequency",
            ),
