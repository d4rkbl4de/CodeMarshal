...
class ViewRegistry:
    ...
    def get_view_class(cls, view_type: ViewType) -> type:
        ...
        if view_type not in cls._registered_views:
            raise ValueError(
                f"View type '{view_type}' is not registered. "
    ...
    def create_view(cls, view_type: ViewType, *args, **kwargs) -> Any:
        ...
        try:
            ...
            raise TypeError(f"Failed to create view '{view_type}': {e}") from e
    ...
    def check_view_compliance(cls, view_type: ViewType) -> dict[str, Any]:
        ...
        report["checks"]["required_methods"] = {
            ...
            "missing": [
                method for method in required_methods if not hasattr(view_class, method)
            ],
        }
        ...
        if hasattr(view_class, "render"):
            ...
            if hasattr(render_method, "__annotations__"):
                ...
                report["checks"]["render_return_type"] = {
                    "passed": return_annotation == dict[str, Any]
                    or "Dict" in str(return_annotation),
...
def get_view(view_type: str) -> type | None:
    """
...
def validate_view_transition(
    current_view: ViewType | None, next_view: ViewType
) -> tuple[bool, str]:
    """
    ...
    if next_view in allowed_transitions.get(current_view, []):
        return (
            True,
            f"Valid transition from {current_view.display_name} to {next_view.display_name}",
        )
    else:
        return (
            False,
            f"Cannot transition from {current_view.display_name} to {next_view.display_name}",
        )
...
def _verify_exports() -> None:
    ...
    for view_type in ViewType:
        ...
        assert view_class.__name__ in __all__, (
            f"View class {view_class.__name__} not exported"
        )

    # Verify no ad-hoc exports
    exported_classes = [
        globals()[name] for name in __all__ if isinstance(globals().get(name), type)
    ]
    for cls in exported_classes:
        if cls.__module__.startswith("lens.views."):
            assert cls.__name__ in __all__, (
                f"View class {cls.__name__} from lens.views not exported"
            )
