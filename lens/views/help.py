...
class HelpDatabase:
    ...
    @classmethod
    def get_entry(cls, entry_id: str) -> Optional[HelpEntry]:
...
class HelpView:
    ...
    def __init__(
        ...
        config: Optional[HelpRenderConfig] = None
    ) -> None:
        """
        ...
        self._current_entry_id: Optional[str] = current_entry_id
    ...
    def _render_overview(self) -> Dict[str, Any]:
        ...
        category_overviews.sort(key=lambda c: next(
            s.display_order for s in sections 
            if s.category.name == c["category"]
        ))
    ...
    def _render_single_entry(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "context_hints": self._get_context_hints(entry) if self._config.show_context_hints else None,
            "is_primary": True
        }
    ...
    def _get_context_hints(self, entry: HelpEntry) -> Optional[List[str]]:
        ...
        if entry.requires_context:
            if not self._context.current_focus and "focus" in entry.content.lower():
                hints.append("You don't have a current focus. Set one to use this feature.")
            
            if not self._context.investigation_path and "investigation" in entry.content.lower():
        ...
        if entry.entry_type == HelpEntryType.WARNING:
            hints.append("This is a warning about potential misuse or misunderstanding.")
        ...
        if entry.category == HelpCategory.TROUBLESHOOTING:
            hints.append("If problem persists, check the constitutional rules for deeper issues.")
    ...
    def get_quick_reference(self) -> Dict[str, Any]:
        ...
        for category in HelpCategory:
            ...
            if entries:
                ...
                concept_entries = [e for e in entries if e.entry_type in 
                                 (HelpEntryType.CONCEPT, HelpEntryType.CONSTRAINT)]
                if concept_entries:
    ...
    def validate_integrity(self) -> List[str]:
        ...
        for section in HelpDatabase.get_sections():
            for entry in section.entries:
                ...
                code_keywords = ["should refactor", "better to", "recommend", "best practice"]
                for keyword in code_keywords:
                    if keyword in entry.content.lower():
                        violations.append(f"Entry {entry.id} contains domain advice: '{keyword}'")
        ...
        for section in HelpDatabase.get_sections():
            for entry in section.entries:
                for keyword in shortcut_keywords:
                    if keyword in entry.content.lower():
                        violations.append(f"Entry {entry.id} suggests shortcuts: '{keyword}'")
        ...
        constraint_entries = [
            e for s in HelpDatabase.get_sections()
            for e in s.entries
