...
class ObservationGroup:
    ...
    @property
    def earliest_observation(self) -> datetime | None:
...
class ExaminationView:
    ...
    def __init__(
        ...
        observations: tuple[RawObservation, ...],
        config: ExaminationRenderConfig | None = None,
    ) -> None:
        """
        ...
        self._config: ExaminationRenderConfig = (
            config or ExaminationRenderConfig.default()
        )

        # Apply philosophy rules
        self._apply_philosophy_rules()

    def _validate_observations_are_raw(
        self, observations: tuple[RawObservation, ...]
    ) -> None:
        """
        ...
        for obs in observations:
            ...
            for forbidden_field in forbidden_fields:
                if hasattr(obs, forbidden_field):
                    raise ValueError(f"Observation contains analysis field '{forbidden_field}'")
            ...
            for phrase in interpretive_phrases:
                if phrase in content_lower:
                    raise ValueError(
                        f"Observation content contains interpretation: '{phrase}'"
                    )
    ...
    def _organize_chronologically(self) -> dict[str, Any]:
        ...
        sorted_obs = sorted(
            self._observations, key=lambda o: o.observed_at, reverse=True
        )
        ...
        return {
            ...
            "time_range": {
                "earliest": sorted_obs[-1].observed_at.isoformat()
                if sorted_obs
                else None,
    ...
    def _organize_by_type(self) -> dict[str, Any]:
        ...
        return {"organization": "by_type", "groups": groups}
    ...
    def _organize_by_source(self) -> dict[str, Any]:
        ...
        for source_path in sorted(groups_by_source.keys()):
            ...
            sorted_obs = sorted(
                source_obs, key=lambda o: (o.line_number or 0, o.column_number or 0)
            )
        ...
        return {"organization": "by_source", "groups": groups}
    ...
    def _organize_contextually(self) -> dict[str, Any]:
        ...
        if other_obs:
            other_by_type: dict[ObservationType, list[RawObservation]] = defaultdict(
                list
            )
            ...
            for obs_type in ObservationType:
                if obs_type in other_by_type:
                    ...
                    # Apply per-group limit
                    display_type = sorted_type[
                        : self._config.max_observations_displayed // 2
                    ]
    ...
    def _prepare_observation_display(self, obs: RawObservation) -> dict[str, Any]:
        ...
        display: dict[str, Any] = {
            ...
            "content": obs.content
            if self._config.show_raw_content
            else "[content hidden]",
            "is_binary": obs.is_binary,
            "content_indicator": self._BINARY_INDICATOR
            if obs.is_binary
            else self._TEXT_INDICATOR,
        }
        ...
        if (
            self._config.highlight_current_focus
            and self._context.current_focus
            and self._context.current_focus in obs.source_path
        ):
    ...
    def _prepare_group_display(self, group: ObservationGroup) -> dict[str, Any]:
        ...
        display: dict[str, Any] = {
            ...
            "observations": [
                self._prepare_observation_display(obs) for obs in group.observations
            ],
        ...
        if (
            group.group_type == "time_based"
            and group.earliest_observation
            and group.latest_observation
        ):
            display["time_span"] = {
    ...
    def _apply_display_rules(self, organized: dict[str, Any]) -> dict[str, Any]:
        ...
        result["display_stats"] = {
            ...
            if displayed_count < total_observations
            else None,
        }
    ...
    def get_filtered_view(
        self,
        filter_type: ObservationType | None = None,
        ...
        return ExaminationView(
            context=self._context, observations=tuple(filtered_obs), config=new_config
        )
    ...
    def create_test_view(cls) -> ExaminationView:
        ...
        return cls(TestContext(), (obs1, obs2, obs3, obs4, obs5))
...
def main() -> None:
    ...
    # Test empty state
    print("\n=== EMPTY STATE TEST ===")
    empty_view = ExaminationView(view._context, ())
