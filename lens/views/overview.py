...
class InvestigationTimeline:
    ...
    def __post_init__(self) -> None:
        ...
        if self.last_activity and self.last_activity.tzinfo is None:
            object.__setattr__(
                self, "last_activity", self.last_activity.replace(tzinfo=UTC)
            )
...
class OverviewView:
    ...
    def __init__(
        ...
        config: OverviewRenderConfig | None = None,
    ) -> None:
        """
        ...
        if not isinstance(metrics, InvestigationMetrics):
            raise TypeError(
                f"metrics must be InvestigationMetrics, got {type(metrics)}"
            )
        ...
        self._current_focus: CurrentFocus | None = current_focus
        self._unknowns: tuple[UnknownItem, ...] = unknowns
        self._timeline: InvestigationTimeline | None = timeline
    ...
    def _determine_stage(self) -> InvestigationStage:
        ...
        if (
            self._metrics.has_patterns
            and self._metrics.notes_recorded < self._metrics.patterns_detected
        ):
    ...
    def _calculate_stage_progress(self, stage: InvestigationStage) -> float | None:
        ...
        if stage == InvestigationStage.ORIENTATION:
            ...
            if self._metrics.observations_total == 0:
                return 0.0
            return min(
                self._metrics.observation_coverage * 2, 1.0
            )  # Scale to 50% coverage = complete
            ...
            if not self._metrics.has_patterns:
                return 0.0
            notes_per_pattern = self._metrics.notes_recorded / max(
    ...
    def _render_timeline(self) -> dict[str, Any]:
        ...
        return {
            ...
            "content": {
                ...
                "last_activity": self._timeline.last_activity.isoformat()
                if self._timeline.last_activity
                else None,
    ...
    def _render_unknowns(self) -> dict[str, Any]:
        ...
        if has_more:
            content["message"] = (
                f"Showing {len(display_unknowns)} of {len(self._unknowns)} unknowns"
            )
    ...
    def _get_recommended_action(self, stage: InvestigationStage) -> str | None:
        """
    ...
    def _apply_single_focus(self, overview: dict[str, Any]) -> dict[str, Any]:
        ...
        primary_sections = [
            s for s in overview["sections"] if s.get("is_primary", False)
        ]
    ...
    def validate_integrity(self) -> list[str]:
            ...
            if isinstance(content, dict):
                ...
                for phrase in interpretative_phrases:
                    if phrase in str(content).lower():
                        violations.append(
                            f"Section contains interpretative language: '{phrase}'"
                        )

        # Check 2: Unknowns must be shown if present
        if self._unknowns and not self._config.show_unknowns:
            violations.append(
                "Unknown items exist but are hidden from view (Article 3 violation)"
            )

        # Check 3: Must show what doesn't exist
        if not self._config.show_counts:
            violations.append(
                "Presence indicators disabled - user cannot see what exists"
            )
        ...
        primary_sections = [
            s for s in rendered.get("sections", []) if s.get("is_primary", False)
        ]
        if len(primary_sections) != 1:
            violations.append(
                f"Single-focus violation: {len(primary_sections)} primary sections"
            )
    ...
    def create_test_view(cls) -> OverviewView:
        ...
        timeline = InvestigationTimeline(
            ...
            milestones=(
                datetime.now(UTC) - timedelta(hours=1, minutes=45),
                datetime.now(UTC) - timedelta(hours=1, minutes=15),
            ),
        )

        return cls(
            TestContext(), TestNoteCollection(), metrics, focus, unknowns, timeline
        )
...
def main() -> None:
    ...
    empty_view = OverviewView(
        ...
        OverviewRenderConfig(
            show_counts=True, show_unknowns=True, show_next_actions=True
        ),
    )
