...
class LoadingIndicator:
    ...
    @classmethod
    def create_idle(cls, context: str | None = None) -> "LoadingIndicator":
        """Create an idle indicator."""
        return cls(state=LoadingState.IDLE, context=context)

    @classmethod
    def create_working(
        cls, context: str | None = None, with_timer: bool = False
    ) -> "LoadingIndicator":
        """Create a working indicator."""
        start_time = time.time() if with_timer else None
        return cls(state=LoadingState.WORKING, context=context, start_time=start_time)
    ...
    def create_blocked(
        ...
        start_time = time.time() if with_timer else None
        return cls(state=LoadingState.BLOCKED, context=context, start_time=start_time)

    @classmethod
    def create_waiting_for_input(cls, context: str | None = None) -> "LoadingIndicator":
        """Create a waiting-for-input indicator."""
        return cls(state=LoadingState.WAITING_ON_INPUT, context=context)
    ...
    def decorator(func: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                print(
                    f"[PERFORMANCE] {operation_name}: {monitor.human_readable_duration}"
                )
