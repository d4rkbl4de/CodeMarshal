...
from typing import Protocol, runtime_checkable, Optional, Dict, List, FrozenSet, Any
...
class ClarityRule:
    ...
    def validate_content(self, content: TruthfulContent) -> List[str]:
        ...
        for signal in self.required_signals:
            ...
            if signal == "WARNING_SYMBOL" and not content.status_explanation.startswith("⚠️"):
        ...
        for signal in self.prohibited_signals:
            if signal == "ABSOLUTE_CERTAINTY" and content.confidence_level == 1.0:
                violations.append(f"Prohibited absolute certainty for {self.status.name}")
        ...
        if self.mandatory_message and self.mandatory_message not in content.status_explanation:
            violations.append(f"Missing mandatory message for {self.status.name}: {self.mandatory_message}")
...
class ClarityValidator:
    ...
    _CLARITY_RULES: Dict[EpistemicStatus, ClarityRule] = {
        ...
        EpistemicStatus.AMBIGUOUS: ClarityRule(
            ...
            required_signals=frozenset({"WARNING_SYMBOL", "ALTERNATIVE_INTERPRETATIONS"}),
    ...
    def __init__(self) -> None:
        ...
        for status in EpistemicStatus:
            if status not in self._CLARITY_RULES:
                raise ValueError(f"Missing clarity rule for epistemic status: {status.name}")
    ...
    def _validate_global_constraints(self, content_list: List[TruthfulContent]) -> None:
        ...
        uncertain_count = sum(1 for c in content_list if c.epistemic_status.requires_warning)
...
class PerformanceTracker:
    ...
    def start(self) -> PerformanceMarker:
        ...
        marker = PerformanceMarker(start_time=datetime.now(timezone.utc))
        self.markers.append(marker)
        return marker
    
    def end(self, marker: PerformanceMarker, resource_usage: Optional[Dict[str, Any]] = None) -> None:
        """
...
def validate_before_display(content_list: List[TruthfulContent]) -> List[str]:
    ...
    for content in content_list:
        ...
        if content.epistemic_status == EpistemicStatus.KNOWN and not content.evidence_references:
