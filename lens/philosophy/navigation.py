...
from typing import Protocol, runtime_checkable, Optional, FrozenSet, Dict, List
...
class InvestigationContext(Protocol):
    ...
    @property
    def current_question_type(self) -> Optional[str]:
        """Type of the most recent human question, if any."""
        ...
    
    @property
    def current_focus_anchor(self) -> Optional[str]:
...
class TransitionRule:
    ...
    from_type: Optional[QuestionType]  # None = initial state
...
class NavigationViolation(Exception):
    ...
    def __init__(
        self,
        from_type: Optional[QuestionType],
        to_type: QuestionType,
...
class NavigationRule:
    ...
    _TRANSITION_RULES: Dict[Optional[QuestionType], Dict[QuestionType, TransitionRule]] = {
    ...
    def validate_transition(
        ...
        if current_str:
            ...
            if current_enum is None:
                ...
                raise ValueError(f"Invalid current question type in context: {current_str}")
...
class MockInvestigationContext:
    ...
    current_focus_anchor: Optional[str] = None
