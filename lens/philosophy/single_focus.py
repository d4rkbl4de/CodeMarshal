...
from typing import Protocol, runtime_checkable, Optional, Set, List, FrozenSet, Union, Sequence
...
class InterfaceIntent(Protocol):
    ...
    @property
    def primary_focus(self) -> Optional[FocusableContent]:
...
class SingleFocusViolation(Exception):
    ...
    def __init__(
        ...
        if len(primary_foci) == 0:
            ...
            foci_names = ", ".join(
                f.name if isinstance(f, PrimaryContentType) else f 
                for f in primary_foci
            )
            msg = (
...
class SingleFocusRule:
    ...
    COMPETING_PAIRS: FrozenSet[FrozenSet[PrimaryContentType]] = frozenset({
        frozenset({PrimaryContentType.OBSERVATION_SNAPSHOT, PrimaryContentType.QUESTION_ANSWER}),
        frozenset({PrimaryContentType.PATTERN_REPORT, PrimaryContentType.NOTEBOOK_ENTRY}),
        frozenset({PrimaryContentType.INVESTIGATION_OVERVIEW, PrimaryContentType.QUESTION_ANSWER}),
    })
    ...
    def validate_interface_intent(self, intent: InterfaceIntent) -> None:
        ...
        try:
            ...
            raise SingleFocusViolation([primary_type_str], None)
        ...
        if competing_primaries:
            ...
            all_foci: Sequence[Union[PrimaryContentType, str]] = (primary_type,) + tuple(competing_primaries)
    ...
    def _find_competing_primary_content(
        self,
        primary_type: PrimaryContentType,
        secondaries: FrozenSet[FocusableContent]
...
def validate_competing_primaries(
    primary_type: PrimaryContentType,
    candidate_secondary_types: List[str]
) -> List[str]:
    """
...
class MockInterfaceIntent:
    ...
    primary_focus: Optional[FocusableContent]
