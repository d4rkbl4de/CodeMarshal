...
class PerformanceMonitor:
    ...
    def generate_report(self):
        ...
        report = {
            ...
            "average_memory_mb": sum(m.get("memory_mb", 0.0) for m in self.metrics) / len(self.metrics),
            "average_cpu_percent": sum(m.get("cpu_percent", 0.0) for m in self.metrics) / len(self.metrics),
        ...
        for key, value in report.items():
            if "seconds" in key:
                ...
                print(f"{key.replace('_', ' ').title()}: {value:.1f}s ({hours:.2f} hours)")
            elif "memory" in key:
    ...
    def monitor(self):
        ...
        while True:
            try:
                ...
                eta_str = str(eta).split(".")[0] if eta.total_seconds() > 0 else "--:--:--"
